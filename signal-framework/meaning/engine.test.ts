...
describe("meaning integration", () => {
  ...
  it("integrates with Stocks Optimizer and lowers exposure for recovery pressure", () => {
    const source = stockSource("I want to recover my losses quickly.");
    const meaning = evaluateStocksMeaning(source);
    const purpose = evaluateStocksPurpose(source, { meaning });
...
function stockSource(meaningText: string) {
  return {
