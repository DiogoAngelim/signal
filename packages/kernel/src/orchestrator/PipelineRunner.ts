...
export class PipelineRunner {
  ...
  async run(
    ...
    builder.appendTrace(
      this.traceEntry("generate", generator.id, genDuration, {
        inputKeys: Object.keys(input),
      }),
    );
    builder.withFeatures(features);
    this.eventBus.emit(
      "pipeline:generate-complete",
      { packageId, generatorId: generator.id, durationMs: genDuration },
      "PipelineRunner",
    );
    ...
    builder.appendTrace(
      this.traceEntry("analyze", analyzer.id, anaDuration, {}),
    );
    builder.withAnalysis(analysis);
    this.eventBus.emit(
      "pipeline:analyze-complete",
      { packageId, analyzerId: analyzer.id, durationMs: anaDuration },
      "PipelineRunner",
    );
    ...
    builder.appendTrace(
      this.traceEntry("score", scorer.id, scoreDuration, {
    ...
    const aggregateResult = await aggregator.aggregate({
      ...scoredResult,
      analysis,
    });
    const aggDuration = Date.now() - aggStart;
    builder.appendTrace(
      this.traceEntry("aggregate", aggregator.id, aggDuration, {
