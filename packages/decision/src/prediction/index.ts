...
export function generatePredictionScenarios(
  ...
  const labels = uniqueStrings(
    input.labels?.length ? input.labels : DEFAULT_LABELS,
  ).slice(0, 6);
  const assumptions = uniqueStrings(
    input.assumptions ?? [
      "Current evidence remains relevant long enough to evaluate.",
    ],
  );
  ...
  const totalProbability =
    raw.reduce((sum, scenario) => sum + scenario.probability, 0) || 1;
...
export function mostLikelyScenario(
  scenarios: readonly PredictionScenario[],
): PredictionScenario | undefined {
  return [...scenarios].sort((a, b) => b.probability - a.probability)[0];
}

export function mostDangerousScenario(
  scenarios: readonly PredictionScenario[],
): PredictionScenario | undefined {
  return [...scenarios].sort(
    (a, b) =>
      b.downsideRisk - a.downsideRisk || b.expectedRisk - a.expectedRisk,
  )[0];
}

export function bestPurposeScenario(
  scenarios: readonly PredictionScenario[],
): PredictionScenario | undefined {
  return [...scenarios].sort(
    (a, b) =>
      b.purposeAlignment - a.purposeAlignment ||
      b.needAlignment - a.needAlignment,
  )[0];
}
...
function scenarioFor(input: {
  ...
  const uncertainty = clamp(
    input.uncertainty + (input.index % 2 === 0 ? -6 : 8),
  );
  const downsideRisk = clamp(
    risk * 0.72 + uncertainty * 0.28 + (polarity < 0 ? 12 : 0),
  );
  const confidence = clamp(
    input.confidence + polarity * 8 - uncertainty * 0.08,
  );
  ...
  return {
    ...
    label: input.label,
    probability: clamp(
      confidence * 0.5 + (100 - uncertainty) * 0.3 + (100 - risk) * 0.2,
      5,
      95,
    ),
    ...
    purposeAlignment: Math.round(
      clamp(input.purpose + polarity * 10 - downsideRisk * 0.05),
    ),
    needAlignment: Math.round(
      clamp(input.need + polarity * 8 - downsideRisk * 0.04),
    ),
...
function scenarioPolarity(label: string, index: number): number {
  if (/improve|succeed|recover|strength|upside|favorable/i.test(label))
    return 1;
...
function warningSigns(
  ...
  const signs = [
    `Warning signs for "${label}" should be monitored before action scale increases.`,
  ];
  if (risk >= 65) signs.push("Expected risk is elevated.");
  if (uncertainty >= 65) signs.push("Uncertainty is high.");
  if (downsideRisk >= 70)
    signs.push("Downside could overwhelm the expected reward.");
  return signs;
}
