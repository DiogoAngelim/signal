...
describe("assessStewardship", () => {
  it("handles empty input with a stable domain-agnostic contract", () => {
    ...
    expect(assessment.uncertaintySummary).toContain(
      "Decision memory is missing or not yet useful.",
    );
  ...
  it("converts memory, governance, context, and reviews into a gradual assessment", () => {
        ...
        evidence: [
          {
            ...
            summary:
              "Independent inspection found the protective control working.",
            quality: "strong",
            durability: "strong",
          },
        ],
    ...
    expect(assessment.whatMatters[0]).toContain(
      "Water system should remain safe, useful, and available.",
    );
    ...
    expect(assessment.smallestResponsibleNextStep.description).toContain(
      "smallest reversible step",
    );
    expect(assessment.monitoringPlan[0]).toBe(
      "Use cadence: Review every week.",
    );
  });
  ...
  it("monitors strong evidence when material uncertainty remains", () => {
    const assessment = assessStewardship({
      ...
      context: {
        accountabilityOwner: "Maintainer",
        policyCompliance: "compliant",
        reversibility: "high",
      },
      evidence: [
    ...
    expect(assessment.uncertaintySummary.join(" ")).toContain(
      "downstream effect",
    );
  });
  ...
  it("stops on policy violation without claiming certainty", () => {
    const assessment = assessStewardship({
      ...
      context: {
        ...
        accountabilityOwner: "Safety lead",
      },
      threats: [
        {
          id: "threat:critical",
    ...
    expect(assessment.disclaimers.join(" ")).toContain(
