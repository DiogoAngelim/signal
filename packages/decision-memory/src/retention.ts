...
import type { DecisionMemoryConfig, ExpiredMemoryMode, RetentionPolicy, RetentionTier } from "./types";
...
export function retentionPolicyFromEnv(env: NodeJS.ProcessEnv = process.env): RetentionPolicy {
  return {
    hotDays: positiveInt(env["SIGNAL_MEMORY_RETENTION_HOT_DAYS"], DEFAULT_RETENTION_POLICY.hotDays),
    warmDays: positiveInt(env["SIGNAL_MEMORY_RETENTION_WARM_DAYS"], DEFAULT_RETENTION_POLICY.warmDays),
    coldDays: positiveInt(env["SIGNAL_MEMORY_RETENTION_COLD_DAYS"], DEFAULT_RETENTION_POLICY.coldDays),
    expiredMode: expiredMode(env["SIGNAL_MEMORY_EXPIRED_MODE"]),
  };
}

export function decisionMemoryConfigFromEnv(env: NodeJS.ProcessEnv = process.env): DecisionMemoryConfig {
  const enabled = env["SIGNAL_MEMORY_ENABLED"] !== "false";
  const provider = env["SIGNAL_MEMORY_PROVIDER"] === "postgres" || env["DATABASE_URL"] ? "postgres" : "memory";
  return {
    enabled,
...
export function normalizeRetentionTier(value: unknown, fallback: RetentionTier = "hot"): RetentionTier {
...
export class MemoryLifecycle {
  ...
  tierFor(record: Pick<SignalDecisionRecord, "createdAt">, now: Date = new Date()): RetentionTier {
    return retentionTierForCreatedAt(record.createdAt, this.policy, now);
  }

  apply(record: SignalDecisionRecord, now: Date = new Date()): SignalDecisionRecord {
...
export function normalizeRetentionPolicy(policy: Partial<RetentionPolicy> = {}): RetentionPolicy {
  ...
  const warmDays = Math.max(hotDays, positiveInt(policy.warmDays, DEFAULT_RETENTION_POLICY.warmDays));
  const coldDays = Math.max(warmDays, positiveInt(policy.coldDays, DEFAULT_RETENTION_POLICY.coldDays));
  return {
    hotDays,
    warmDays,
    coldDays,
