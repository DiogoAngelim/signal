  ...
  [
    {
      ...
      version: "v1",
      description:
        "Evaluate generic decision trust, constraints, resources, and policy into a recommended commitment.",
      idempotent: true,
      replaySafe: true,
    },
  ];
...
export const commitmentEvaluateResultSchema: CommitmentSchema<CommitmentResult> =
  {
    parse(value: unknown): CommitmentResult {
      ...
      if (
        ...
        throw new Error(
          "commitment.evaluate.v1 result did not match the protocol contract",
        );
      }
      return result;
    },
...
export function registerCommitmentOperations(
  registry: CommitmentRuntimeLike,
): CommitmentOperationDefinition[] {
  const definition = COMMITMENT_OPERATION_DEFINITIONS[0];
  if (!definition) {
    throw new Error("No commitment operation definitions found");
  }
