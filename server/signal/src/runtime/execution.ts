...
export function throwIfExecutionBlocked(request: SignalRequestContext): void {
  if (request.deadlineAt && Date.parse(request.deadlineAt) < Date.now()) {
    throw createProtocolError("TIMEOUT", "Execution deadline exceeded", {
      details: {
        deadlineAt: request.deadlineAt,
      },
    });
  ...
  if (request.abortSignal?.aborted) {
    throw createProtocolError("EXECUTION_FAILED", "Execution cancelled", {
      details: {
        reason:
      ...
      error instanceof Error ? error.message : fallbackMessage,
      {
        ...
        messageId:
          typeof (error as { messageId?: unknown }).messageId === "string"
            ? (error as { messageId?: string }).messageId
            : context?.messageId,
        details:
