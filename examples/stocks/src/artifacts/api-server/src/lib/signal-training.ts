...
const CACHE_TTL_MS = Number(
  process.env.STOCK_SIGNAL_TRAINING_CACHE_TTL_MS ?? 60_000,
);
...
const stateCache = new Map<
  string,
  { expiresAt: number; state: SignalTrainingState }
>();
let schemaReady: Promise<void> | null = null;
let trainingPersistenceWarningLogged = false;
...
function logTrainingPersistenceWarning(error: unknown, message: string) {
  if (trainingPersistenceWarningLogged) {
    return;
  }

  trainingPersistenceWarningLogged = true;
...
function mapRowToState(
  ...
  return {
    ...
    lastSignalConfidence: Math.round(
      toNumber(row.last_signal_confidence, fallback.lastSignalConfidence),
    ),
    lastSignalSource:
      row.last_signal_source === "node-ecu" ? "node-ecu" : "heuristic",
    lastSignalEmittedAt:
    ...
    lastSignalEntryPrice: toNumber(
      row.last_signal_entry_price,
      fallback.lastSignalEntryPrice,
    ),
    lastObservedPrice: toNumber(
      row.last_observed_price,
      fallback.lastObservedPrice,
    ),
    lastObservedAt:
    ...
    buyObservations: Math.max(
      0,
      Math.round(toNumber(row.buy_observations, fallback.buyObservations)),
    ),
    buySuccesses: Math.max(
      0,
      Math.round(toNumber(row.buy_successes, fallback.buySuccesses)),
    ),
    holdObservations: Math.max(
      0,
      Math.round(toNumber(row.hold_observations, fallback.holdObservations)),
    ),
    holdSuccesses: Math.max(
      0,
      Math.round(toNumber(row.hold_successes, fallback.holdSuccesses)),
    ),
    sellObservations: Math.max(
      0,
      Math.round(toNumber(row.sell_observations, fallback.sellObservations)),
    ),
    sellSuccesses: Math.max(
      0,
      Math.round(toNumber(row.sell_successes, fallback.sellSuccesses)),
    ),
...
function getActionStats(
  ...
  action: TradeSignal,
): { observations: number; successes: number } {
  switch (action) {
    case "Buy":
      return {
        observations: state.buyObservations,
        successes: state.buySuccesses,
      };
    case "Sell":
      return {
        observations: state.sellObservations,
        successes: state.sellSuccesses,
      };
    default:
      return {
        observations: state.holdObservations,
        successes: state.holdSuccesses,
      };
  }
}

function getActionSuccessRate(
  state: SignalTrainingState,
  action: TradeSignal,
): number | null {
...
async function saveSignalTrainingState(
  state: SignalTrainingState,
): Promise<void> {
...
export async function getSignalTrainingState(
  market: string,
  symbol: string,
): Promise<SignalTrainingState> {
  ...
  try {
    ...
    logTrainingPersistenceWarning(
      error,
      "Signal training persistence unavailable; using default signal calibration",
    );
    state = createDefaultState(normalizedMarket, normalizedSymbol);
  }
...
export function calibrateSignalDecision(
  ...
  return {
    ...
    signalConfidence: Math.round(
      clamp(
        decision.signalConfidence + state.confidenceBias + actionBias,
        15,
        99,
      ),
    ),
...
export async function recordSignalSnapshot(input: {
  ...
  const hasPreviousEmission =
    Boolean(state.lastSignalEmittedAt) && state.lastSignalEntryPrice > 0;
  ...
  try {
    ...
    logTrainingPersistenceWarning(
      error,
      "Signal training persistence unavailable; returning in-memory signal snapshot",
    );
    stateCache.set(cacheKey(state.market, state.symbol), {
      expiresAt: Date.now() + CACHE_TTL_MS,
      state: { ...state },
