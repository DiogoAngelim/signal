...
const currentSituation: StockAllocationSituation = {
  ...
  marketRiskPct: 61,
  liquidityRiskPct: 32,
  investmentThesis: "The thesis is improving, but the allocation should stay reviewable.",
...
const reviewedHistory: ReviewedInvestmentSituation[] = [{
  ...
  postmortem: "A prior recovery allocation worked only after exposure stayed capped.",
  investmentLesson: "Keep exposure capped until liquidity and momentum both survive review.",
...
describe("Signal learning adapter", () => {
  it("maps stock concepts into Signal contracts at the app boundary", () => {
    const input = mapStockAllocationToSignalInput(currentSituation, reviewedHistory);
    ...
    expect(input.threats?.map((threat) => threat.type)).toEqual(["Threat", "Threat"]);
    ...
    expect(input.relationships?.some((relationship) => (
  ...
  it("uses reviewed history before a new investment outcome exists", () => {
    const judgment = evaluateStocksLearningJudgment(currentSituation, reviewedHistory);

    expect(judgment.signalResult.reviewedHistory.reviewRefs).toHaveLength(1);
    expect(judgment.signalResult.similarityMatches[0]?.score).toBeGreaterThan(0);
    expect(judgment.signalResult.judgment.futureOutcomeRequired).toBe(false);
    expect(judgment.signalResult.judgment.reviewedHistoryRefs).toContain("reviewed-history:current");
    expect(judgment.signalResult.judgment.lessonRefs).toContain("lesson:xyz-prior-review");
    ...
    expect(judgment.reviewableLanguage.join(" ")).toContain("Previously reviewed situations suggest");
  });
