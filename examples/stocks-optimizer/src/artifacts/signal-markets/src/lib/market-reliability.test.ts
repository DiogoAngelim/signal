...
function stock(overrides: Record<string, any> = {}) {
  return {
    ...
    history: [
      { date: "2026-05-26", open: 98, high: 101, low: 97, close: 100, volume: 900_000 },
      { date: "2026-05-27", open: 100, high: 102, low: 99, close: 101, volume: 1_000_000 },
    ],
    ...overrides,
  };
}

function source(overrides: Partial<MarketReliabilitySource> = {}): MarketReliabilitySource {
  return {
    market: "US",
    marketStatus: "Open",
    stocks: [stock({ ticker: "AAA" }), stock({ ticker: "BBB", price: 42 }), stock({ ticker: "CCC", price: 18 })],
    avgRisk: 36,
    avgQuality: 68,
    breadth: 45,
    confidence: 72,
...
describe("stocks optimizer market reliability policy", () => {
  it("keeps a synchronized closed venue healthy while explaining the pause", () => {
    const result = evaluateMarketReliability(source({ marketStatus: "Closed" }));
    ...
    expect(result.diagnostics.map((item) => item.code)).toContain("VENUE_CLOSED");
  });
  ...
  it("does not coerce closed signal-only coverage into invalid price ranges", () => {
    ...
    expect(result.diagnostics.map((item) => item.code)).not.toContain("FIELD_OUT_OF_RANGE");
  });
    ...
    const result = evaluateMarketReliability(
      source({
        ...
        stocks: [
          ...
          stock({ ticker: "OLD2", updatedAt: new Date(now - 600_000).toISOString() }),
        ],
  ...
  it("detects missing OHLCV fields, missing volume, duplicate candles, and low synchronized samples", () => {
        ...
        stocks: [
          stock({
            ...
            history: [
              ...
              { date: "2026-05-27", open: 10, high: 11, low: 9, close: 10, volume: 20 },
            ],
          }),
        ],
    ...
    expect(result.diagnostics.map((item) => item.code)).toEqual(
      expect.arrayContaining(["MISSING_VOLUME", "MISSING_OHLCV", "DUPLICATED_CANDLES", "LOW_SYNCHRONIZED_CANDLE_COUNT"]),
    );
  });
  ...
  it("distinguishes insufficient coverage and low breadth from bearish signals", () => {
    ...
    expect(result.status).toBe("insufficient");
    expect(result.market.primaryIssues).toEqual(expect.arrayContaining(["Low ticker coverage"]));
    expect(result.market.primaryIssues).not.toContain("Low breadth participation");
    expect(result.diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: "BREADTH_PARTICIPATION_LOW", severity: "info" }),
      ]),
    );
  ...
  it("keeps high coverage market data healthy when only a small tail is unavailable", () => {
    ...
    const stocks = [
      ...Array.from({ length: 68 }, (_, index) => stock({ ticker: `OK${index}`, price: 10 + index, signalStatus: "missing" })),
      unavailable("MISS1"),
      unavailable("MISS2"),
      unavailable("MISS3"),
    ];
    ...
    expect(result.diagnostics).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ code: "PARTIAL_API_FAILURE", severity: "info", observed: 3 }),
      ]),
    );
  });
  ...
  it("activates defensive mode for desync, partial API failure, fallback, and synthetic data", () => {
    const result = evaluateMarketReliability(
      source({
        ...
        stocks: [
          stock({ ticker: "PENDING", quoteStatus: "pending", source: "pending" }),
          stock({ ticker: "PAUSED", quoteStatus: "paused", source: "paused" }),
          stock({ ticker: "FAIL", quoteStatus: "unavailable", source: "tradingview-failed:502" }),
          stock({ ticker: "FALL", quoteSource: "fallback-cache" }),
          stock({ ticker: "SYN", sourceFile: "mock-data.json", name: "Demo asset" }),
        ],
  ...
  it("renders missing data as uncertainty and caps downstream confidence", () => {
    const reliability = evaluateMarketReliability(
      source({
        stocks: [{ ticker: "MISSING", quoteStatus: "unavailable", source: "failed" }],
    ...
    const metrics = applyReliabilityToMetricInputs(
      [
        { key: "dataReliability", value: 0, raw: 0, confidence: 100, detail: "Raw 0.0%" },
    ...
    expect(metrics.every((metric) => (metric.confidence ?? 100) <= reliability.confidenceCap)).toBe(true);
    expect(capReliabilityConfidence(75, reliability)).toBe(reliability.confidenceCap);
    expect(capReliabilityConfidence(null, reliability)).toBeNull();
    expect(capReliabilityExposure(50, reliability)).toBe(50 * (reliability.confidenceCap / 100));
  ...
  it("handles open healthy data and invalid price ranges deterministically", () => {
    ...
    expect(invalid.diagnostics.map((item) => item.code)).toContain("FIELD_OUT_OF_RANGE");
  });
  ...
  it("accepts valid Binance dust and fiat-denominated crypto prices", () => {
    const result = evaluateMarketReliability(
      source({
        market: "BINANCE",
        stocks: [
          stock({ ticker: "ADXBTC", price: 8.7e-7, history: [8.5e-7, 8.7e-7], signalStatus: "missing" }),
          stock({ ticker: "BTCIDR", price: 1_310_431_087, history: [1_300_000_000, 1_310_431_087], signalStatus: "missing" }),
          stock({ ticker: "BTCTRY", price: 3_386_240, history: [3_350_000, 3_386_240], signalStatus: "missing" }),
        ],
    ...
    expect(result.market.rejectedAssets).toBe(0);
    expect(result.diagnostics.map((item) => item.code)).not.toContain("FIELD_OUT_OF_RANGE");
    expect(result.diagnostics.map((item) => item.code)).not.toContain("FIELD_OUTLIER");
  });

  it("explains degraded source quality and accepts Date timestamps", () => {
    const result = evaluateMarketReliability(
      source({
        stocks: [stock({ ticker: "DATE", updatedAt: new Date(now - 1_000), sourceQuality: 40 })],
        expectedAssetCount: 1,
      }),
    );
  ...
  it("normalizes sparse identifiers, source labels, invalid timestamps, and uncapped metric confidence", () => {
    const result = evaluateMarketReliability({
      ...
      stocks: [
        ...
        },
        {
          ...
          history: [
            { time: "2026-05-28T14:00:00.000Z", open: 33, high: 35, low: 32, close: 34, volume: 20 },
            { timestamp: "2026-05-28T14:01:00.000Z", open: 34, high: 36, low: 33, close: 35, volume: 21 },
    ...
    const adjusted = applyReliabilityToMetricInputs([{ key: "trendStrength", value: 60, raw: 60 }], result);

    expect(result.metadata.inputCount).toBe(5);
    expect(adjusted[0].confidence).toBe(result.confidenceCap);
    expect(capReliabilityExposure(50, { ...result, confidenceCap: Number.POSITIVE_INFINITY })).toBe(0);
  });

  it("handles malformed stock collections as invalid market data", () => {
    const result = evaluateMarketReliability({ ...source(), stocks: null as unknown as Array<Record<string, any>> });
  ...
  it("handles undefined market codes and invalid numeric fields", () => {
    const result = evaluateMarketReliability(
      source({
        market: undefined,
        stocks: [stock({ ticker: "BADNUM", price: "not-a-number", history: ["bad", "worse"] })],
        expectedAssetCount: 1,
      }),
    );
