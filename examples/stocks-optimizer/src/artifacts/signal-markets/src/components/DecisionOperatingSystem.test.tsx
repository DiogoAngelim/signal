...
describe("DecisionOperatingSystem states", () => {
  it("renders an action-first success shell with deeper detail collapsed", () => {
    ...
    expect(html).toContain("What deserves protection?");
    expect(html).toContain("Preserve purchasing power");
    expect(html).toContain("Protect emergency reserves");
    expect(html).toContain("What Matters Most");
    expect(html).toContain("Review Threats");
    expect(html).toContain("Translate to Actions");
    expect(html).toContain("Stewardship Review");
    expect(html).toContain("Decide How To Protect");
    expect(html).toContain("Evidence Range");
    expect(html).toContain("58%-84%");
    expect(html).toContain("Threats Not Yet Known");
    expect(html).toContain("Market participation");
    expect(html).toContain("Inflation and idle cash erosion");
    expect(html).toContain("Observe");
    expect(html).toContain("Stewardship Progress");
    expect(html).toContain("Optionality");
    expect(html).toContain("You Remain In Control");
    expect(html).toContain("Signal evaluates stewardship. You decide.");
    expect(html).toContain("Reliability");
    expect(html).toContain("Threat");
    expect(html).toContain("Why it matters");
    expect(html).toContain("Review evidence");
    expect(html).toContain("Current Stewardship Thesis");
    expect(html).toContain("Contradicting Evidence");
    expect(html).toContain("Mind Change Triggers");
    expect(html).toContain("Capital Plan Context");
    expect(html).toContain("No previous decisions have been reviewed yet.");
    expect(html).toContain("Stewardship path");
    expect(html).toContain("Stewardship metrics");
    expect(html).toContain("Stewardship plan");
  ...
  it("translates internal terms before they reach the default decision surface", () => {
    ...
    expect(html).toContain(
      "stewardship option search and decision control confidence check are still being checked.",
    );
    expect(html).toContain(
      "stewardship option search and return to normal size need decision control confirmation.",
    );
  ...
  it("renders the guided no-market state with required actions", () => {
    ...
    expect(html).toContain("What deserves protection?");
    expect(html).toContain("Capital allocation venue");
    expect(html).toContain(
  ...
  it("covers the market decision guide requirements across data modes", () => {
    ...
    expect(degraded).toContain("Threats Not Yet Known");
  });
  ...
  it("places reviewed learning and draft controls inside the decision step", () => {
    const learning = {
      ...props().opportunities[0].learning,
      learningRecords: [
        {
          lesson:
            "Gradual allocation protected capital during uncertain recovery phases.",
          changes: [
            "Keep future allocation gradual until confirmation repeats.",
          ],
        },
        {
          lesson:
            "Gradual allocation protected capital during uncertain recovery phases.",
          changes: [
            "Require repeated confirmation before increasing allocation.",
          ],
        },
      ],
      narrative: {
        ...props().opportunities[0].learning?.narrative,
        whyItMatters:
          "Reviewed outcomes suggest that patient participation is more durable than early conviction.",
        mindChange:
          "Future participation can increase only after reliability keeps improving across reviewed outcomes.",
      },
    };
    ...
    expect(html).toContain('data-testid="guided-panel-decide-what-to-do"');
    expect(html).toContain('data-testid="what-we-learned-card"');
    expect(html).toContain(
      "Gradual allocation protected capital during uncertain recovery phases.",
    );
    expect(html).toContain("2 reviewed decisions");
    expect(html).toContain("LinkedIn reflection draft");
    expect(html).toContain("No auto-posting");
  ...
  it("converts a contribution amount into investment units", () => {
    ...
    try {
      act(() => {
        stepButton(view.container, "decide-what-to-do").click();
      });
      ...
      expect(view.container.textContent).toContain("Commitment");
      expect(view.container.textContent).toContain("Stewardship focus:");
      expect(
        view.container.querySelector('[data-testid="commitment-units"]')
          ?.textContent,
      ).toBe("0.8000");
    } finally {
      view.cleanup();
