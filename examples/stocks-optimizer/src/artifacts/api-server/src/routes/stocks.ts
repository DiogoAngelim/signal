...
import { type SignalScope, storeSignalEvents } from "../lib/signal-backend.js";
import {
...
function loadMarketList(market: string, limit = 80) {
  ...
  return normalizeStockListArray(symbols)
    .slice(0, limit)
    .map((symbol, index) => ({
      symbol,
      ticker: symbol,
...
const signalPersistenceWarningLogged = false;
const monthlyVolatilityPersistenceWarningLogged = false;
...
const STRATEGY_ROUTE_CACHE = new Map<
  string,
  { cachedAt: number; payload: any }
>();
...
function hasSignalEmailTestAccess(req: Request): boolean {
  const secret =
    process.env.SIGNAL_EMAIL_TEST_SECRET?.trim() ||
...
function buildMinimalStrategyPayload(market: string, limitSymbols = 80) {
  const list = loadMarketList(market).length
    ? loadMarketList(market)
    : loadStockList(market);
  const items = normalizeStockListArray(list).slice(
    0,
    Math.max(1, limitSymbols),
  );
  ...
  const signals = items
    .map((item: any, index: number) => {
      ...
      const setupQuality = strategyClamp(
        strategyNumeric(
          item.setupQuality,
          50 + Math.max(0, changePercent) * 4 + Math.max(0, 20 - index) * 0.25,
        ),
      );
      ...
      const riskPressure = strategyClamp(
        strategyNumeric(
          item.riskPressure,
          Math.max(15, 45 - Math.max(0, changePercent) * 2),
        ),
      );
      ...
      const sizingConstraints = [
        ...
        },
        {
          ...
          reason:
            "Cross-timeframe agreement is not confirmed in the local route.",
        },
        {
      ...
      const financialSizing = sizeFinancialExposure({
        ...
        viability: {
          expectedBenefit: strategyClamp(
            setupQuality * 0.6 + Math.max(0, changePercent) * 7,
          ),
      ...
      return {
        ...
        timingQuality: strategyClamp(
          (setupQuality + Math.max(0, changePercent * 8)) / 2,
        ),
  ...
  const buyCount = decisionSignals.filter(
    (signal) => signal.signalAction === "Buy",
  ).length;
  const avgRisk = decisionSignals.length
    ? decisionSignals.reduce(
        (sum, signal) => sum + strategyNumeric(signal.riskPressure),
        0,
      ) / decisionSignals.length
    : 0;
  const avgQuality = decisionSignals.length
    ? decisionSignals.reduce(
        (sum, signal) => sum + strategyNumeric(signal.setupQuality),
        0,
      ) / decisionSignals.length
    : 0;
  ...
  try {
    ...
    try {
      portfolioPayload = await getOrCreateMarketBacktest(market, {
        ...
        persistDiagnostics:
          req.query.persistDiagnostics === "true" ||
          req.body?.persistDiagnostics === true,
        runtimeMode,
      });
      ...
      res
        .status(500)
        .json({
          error: "Failed to load market backtest cache",
          details: String(err),
        });
      return;
    }
    ...
    if (action === "diagnostics") {
      res.json({
        ...
        resolveDiagnostics:
          portfolioPayload.resolveDiagnostics ??
          summary.resolveDiagnostics ??
          null,
      });
      return;
    }
    ...
    if (action === "walk-forward-market" || action === "live-market") {
      ...
      res.json({
        ...
        resolveDiagnostics:
          ...
          null,
        diagnostics: diagnosticsRequested
          ? (portfolioPayload.diagnostics ?? null)
          : undefined,
      });
      return;
    }
...
async function handleStocksMarketsRoute(_req: any, res: any) {
  try {
    ...
    for (const filename of jsonFiles) {
      ...
      try {
        payload = JSON.parse(
          await fs.readFile(path.join(publicDir, filename), "utf8"),
        );
      } catch {
        continue;
      }
      ...
      for (const row of rows) {
        ...
        if (existing) {
          ...
          existing.displayName =
            existing.displayName ?? existing.name ?? existing.code;
    ...
    res.status(500).json({
      error:
        error instanceof Error
          ? error.message
          : "Failed to load markets from stock-list JSON files",
    });
  }
}
  ...
  try {
    ...
    for (const filename of jsonFiles) {
      ...
      for (const row of fileRows) {
        ...
        const symbol = String(
          row?.symbol ?? row?.ticker ?? row?.code ?? row?.name ?? "",
        ).trim();
        if (!symbol) continue;
    ...
    res.status(500).json({
      error:
        error instanceof Error
          ? error.message
          : "Failed to load stocks from JSON files",
    });
  }
}
...
async function handleStocksQuotesRoute(req: any, res: any) {
  try {
    const market = String(req.body?.market ?? req.query.market ?? "")
      .trim()
      .toUpperCase();
    ...
    const timeoutMs = Math.max(
      5_000,
      Number(req.body?.timeoutMs ?? req.query.timeoutMs ?? 45_000),
    );
    ...
    const withSignals =
      req.body?.withSignals !== false && req.query.withSignals !== "false";
    ...
    const quoteSymbols = new Set(
      enriched.map((quote: any) => String(quote.symbol ?? "").toUpperCase()),
    );
    const unavailableSymbols = symbols.filter(
      (symbol: string) => !quoteSymbols.has(symbol.toUpperCase()),
    );
    const responseQuotes = enriched.map((quote: any) => ({
      ...
      regularMarketChangePercent:
        quote.regularMarketChangePercent ?? quote.changePercent,
...
router.get("/stocks/portfolio-decisions", async (req, res, next) => {
  try {
    res.json(
      await listPortfolioDecisionMemory({
...
router.get("/stocks/portfolio-decisions/audit", async (req, res, next) => {
  try {
    res.json(
      await listPortfolioDecisionAudit({
        market: String(req.query.market ?? ""),
        limit: Number(req.query.limit ?? 50),
      }),
    );
...
router.get("/stocks/portfolio-decisions/outcomes", async (req, res, next) => {
  try {
    res.json(
      await listPortfolioDecisionOutcomes({
        market: String(req.query.market ?? ""),
        limit: Number(req.query.limit ?? 50),
      }),
    );
...
router.get("/stocks/history", async (req, res) => {
  try {
    ...
    res.json({
      ...
      sourceStatus: data.length
        ? (data[0]?.sourceStatus ?? "delayed")
        : "unavailable",
    ...
    res.status(500).json({
      error:
        error instanceof Error ? error.message : "Failed to load stock history",
    });
  }
});
