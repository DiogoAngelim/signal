...
export const SignalModulesSchema = z
  .object({
...
export const EmitSignalRequestSchema = z
  ...
  .catchall(z.unknown())
  .transform((value) => value.signal ?? value);
...
export const CreateWebhookSchema = z
  .object({
    ...
    events: z
      .array(z.enum(["signal.emitted", "webhook.test"]))
      .min(1)
      .max(10)
      .default(["signal.emitted"]),
...
export const RedriveQueueSchema = z
  .object({
...
export type RotateWebhookSecretInput = z.infer<
  typeof RotateWebhookSecretSchema
>;
