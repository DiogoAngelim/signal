...
function base(
  overrides: Partial<StockJudgementInput> = {},
): StockJudgementInput {
  return {
    market: "NASDAQ",
    symbol: "AAPL",
...
test("stock judgement maps trades into generic judgement evidence", () => {
  ...
  assert.ok(
    judgement.reasons.some((reason) => reason.includes("Judgement compared")),
  );
});
...
test("stock judgement reduces confidence when profitable similar trades carried survival scars", () => {
  ...
  const scarred = evaluateStockJudgement(
    base({
      riskPressure: 74,
      volatilityPct: 7,
      liquidityScore: 25,
  ...
  assert.ok(
    scarred.reasons.some((reason) =>
      reason.includes("Survival memory penalized confidence"),
    ),
  );
});

test("stock judgement review-gates wait recommendations before near-ruin exposure goes to zero", () => {
  const reviewed = evaluateStockJudgement(
    base({
      survivalMemory: {
...
test("judgement trust and reasons are agency friendly", () => {
  ...
  assert.ok(
    judgementReasons(review).some((reason) =>
      reason.includes("Judgement warning"),
    ),
  );
});
...
test("stock judgement covers shadow, agency approval, and robustness edge branches", () => {
  const partialAgency = evaluateStockJudgement(
    base({
      readiness: {
        ...base().readiness,
        stage: undefined,
  ...
  const negativeShadow = evaluateStockJudgement(
    base({
