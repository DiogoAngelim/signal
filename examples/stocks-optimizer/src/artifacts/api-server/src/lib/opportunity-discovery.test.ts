...
function signal(overrides: Partial<StockOpportunitySignal>): StockOpportunitySignal {
  return {
    symbol: "AAA",
    signalAction: "Hold",
...
test("ranks improving candidates, stores evidence, and emits adaptive sizing diagnostics", () => {
  const signals = [
    signal({ symbol: "AAA", signalAction: "Buy", suggestedExposure: 1, setupQuality: 86, riskPressure: 18, expectedMove: 4 }),
    signal({ symbol: "BBB", signalAction: "Buy", suggestedExposure: 2, setupQuality: 82, riskPressure: 22, expectedMove: 3 }),
    signal({ symbol: "CCC", signalAction: "Hold", setupQuality: 66, riskPressure: 30, expectedMove: 2 }),
    signal({ symbol: "DDD", signalStatus: "active", setupQuality: 70, riskPressure: 28, expectedMove: 1.5 }),
    signal({ symbol: "EEE", signalStatus: "closed", setupQuality: 72, riskPressure: 26, expectedMove: 1.2 }),
    signal({ symbol: "FFF", signalAction: "Sell", setupQuality: 30, riskPressure: 78, expectedMove: -3 }),
  ];
  ...
  assert.equal(result.candidates[0].candidateScore >= result.candidates[1].candidateScore, true);
  ...
  assert.equal(result.discovery.explanation.supportingEvidence.length > 0, true);
  assert.equal(result.discovery.lifecycle.status !== "none", true);
  assert.equal(result.discovery.recommendedNextStep.length > 0, true);
  assert.equal(result.findings.some((finding) => finding.findingId.startsWith("feature:")), true);
  assert.equal(result.findings.some((finding) => finding.findingId === "almost-qualified:persistence"), true);
  assert.equal(result.candidates.some((candidate) => candidate.evidence.some((item) => item.startsWith("Explorer insight:"))), true);
  ...
  assert.equal(buy.progression.some((point) => point.stage === "Eligible" || point.stage === "Sized"), true);
  assert.equal(buy.evidence.some((item) => item.includes("strategy signal")), true);

  const closed = result.candidates.find((candidate) => candidate.symbol === "EEE");
...
test("uses previous candidate progression, object bars, and fallback histories deterministically", () => {
  ...
  const result = discoverStockOpportunities({
    market: "BINANCE",
    signals: [
      signal({ symbol: "AAA", setupQuality: 68, expectedMove: 2, suggestedExposure: 0 }),
      signal({ symbol: undefined, ticker: "GGG", setupQuality: 44, riskPressure: 42, expectedMove: -0.2, history: [], maxPositionPct: 0 }),
      signal({ symbol: "ZERO", ticker: "ZERO", setupQuality: 50, riskPressure: 45, expectedMove: 0, history: [10], volume: 0 }),
      signal({ symbol: "", ticker: "", setupQuality: 99 }),
    ],
    barsBySymbol: { AAA: bars(50, 0.006), ZERO: bars(20, 0).map((bar) => ({ ...bar, volume: 0 })) },
  ...
  assert.equal(result.discovery.missingEvidence.includes("similar closed outcomes"), true);
  assert.equal(result.density.trend === "improving" || result.density.trend === "flat", true);
  ...
  assert.equal(ggg?.adaptiveSizing.decision !== "allowed" || ggg.adaptiveSizing.size >= 0, true);
});
...
test("generic Discovery flags weak breadth without changing candidate output", () => {
  const result = discoverStockOpportunities({
    market: "BINANCE",
    signals: [
      signal({ symbol: "WEAK1", signalAction: "Hold", setupQuality: 25, riskPressure: 82, expectedMove: -3, history: [10, 9, 8] }),
      signal({ symbol: "WEAK2", signalAction: "Sell", setupQuality: 28, riskPressure: 85, expectedMove: -2, history: [12, 11, 10] }),
    ],
    barsBySymbol: {
  ...
  assert.equal(
    result.discovery.explanation.contradictoryEvidence.some((item) => item.label === "Weak breadth confirmation"),
    true,
  );
});
...
test("generic Discovery handles sparse prior outcomes and constraint fallbacks", () => {
  ...
  const result = discoverStockOpportunities({
    signals: [signal({ symbol: "SPARSE", signalAction: "Buy", setupQuality: 72, riskPressure: 64, expectedMove: 2 })],
    barsBySymbol: { SPARSE: bars(32, 0.004) },
    previousCandidates: previous,
    trades: [
      { profitPct: 0 },
      { symbol: "LOSS", profitPct: -2 },
    ],
    needs: [{
      needId: "need-wait",
      category: "wait",
  ...
  assert.equal(result.discovery.missingEvidence.includes("similar closed outcomes"), true);
  assert.equal(result.discovery.contextMatch.some((match) => match.label.includes("previous context")), true);
  assert.equal(result.discovery.recommendedNextStep.length > 0, true);
});
