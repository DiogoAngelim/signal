...
process.env.TRADINGVIEW_DATA_BASE_URL =
  "https://mock.tradingview.local/api/chart-data";
...
function mockTradingViewBars(symbol: string, exchange: string, bars = 1260) {
  ...
  return Array.from({ length: bars }, (_, index) => {
    ...
    price = Math.max(
      1,
      price * (1 + drift + cycle / 20 + shortCycle / 20 + shock),
    );
    ...
    return {
      date: new Date(Date.now() - (bars - 1 - index) * tradingDayMs)
        .toISOString()
        .slice(0, 10),
      timestamp: new Date(
        Date.now() - (bars - 1 - index) * tradingDayMs,
      ).toISOString(),
      open,
      high,
      low,
...
globalThis.fetch = (async (input: RequestInfo | URL) => {
  ...
  const bars = Math.max(
    90,
    Math.min(Number(url.searchParams.get("bars") ?? 3780), 3780),
  );
...
test("long-history dataset scores full and partial coverage", () => {
  ...
  assert.ok(
    full.coverage.availableYears >= 14.5,
    "full history should span roughly 15 years",
  );
  ...
  assert.ok(
    partial.regimeStats.historyDepthScore < full.regimeStats.historyDepthScore,
  );
  assert.ok(summary.historyDepthScore < full.regimeStats.historyDepthScore);
  assert.match(
    summary.explanation,
    /Extended history improves regime awareness/,
  );
});

test("market backtest builds non-empty historical metrics for a market", async () => {
  const cacheDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "signal-market-backtest-"),
  );
  ...
  assert.ok(
    ...
    "summary should carry an update timestamp",
  );
  assert.ok(
    payload.history.length > 30,
    "history should contain enough points for historical metrics",
  );
  assert.ok(
    payload.trades.length > 0,
    "walk-forward trades should not be empty",
  );
  assert.ok(
    payload.trades.every(
      (trade: any) => Number.isFinite(trade.survivalCost) && trade.outcomeClass,
    ),
  ...
  assert.equal(
    Number.isFinite(
      payload.summary.rawAnnualizedSharpe ?? payload.summary.annualizedSharpe,
    ),
    true,
  );
  assert.equal(
    Number.isFinite(
      payload.summary.rawMaxDrawdownPct ?? payload.summary.maxDrawdownPct,
    ),
    true,
  );
  assert.ok(
    payload.summary.maxDrawdownPct > 0,
    "mark-to-market history should produce a real drawdown",
  );
  const flags = new Set(payload.summary.failureFlags ?? []);
  assert.equal(
    flags.has("OVERFIT_PROFIT_FACTOR"),
    false,
    "real closed-trade evidence should clear too-clean profit-factor warnings",
  );
  assert.equal(
    flags.has("OVERFIT_LOW_DRAWDOWN"),
    false,
    "real mark-to-market history should clear too-clean drawdown warnings",
  );
  assert.equal(
    flags.has("NEEDS_FORWARD_SHADOW"),
    false,
    "closed walk-forward trades should satisfy forward evidence",
  );
  assert.equal(
    flags.has("SYNTHETIC_DATA_FOR_PROMOTION"),
    false,
    "real TradingView history should not be flagged synthetic",
  );
  ...
  assert.ok(
    payload.summary.forwardShadow?.confirmedSignalCount > 0,
    "live buy signals should be confirmed for shadow tracking",
  );
  assert.ok(
    payload.summary.forwardShadow?.observedSignalCount > 0,
    "confirmed signals should be recorded as shadow observations",
  );
  assert.equal(payload.summary.forwardShadow?.collectionStatus, "passed");
  assert.ok(
    payload.summary.historyDiagnostics?.historyCoverageYears >= 14.5,
    "summary should consume long-history coverage",
  );
  assert.ok(
    payload.summary.historyDepthScore >= 90,
    "15-year history should produce high depth score",
  );
  assert.ok(
    payload.summary.regimeCoverageScore >= 0,
    "regime coverage score should be present",
  );
  assert.ok(
    payload.summary.regimeDiversityScore >= 0,
    "regime diversity score should be present",
  );
  assert.ok(
    payload.summary.sampleDiversityScore >= 0,
    "sample diversity score should be present",
  );
  assert.equal(payload.summary.coverageStatus, "full");
  assert.ok(
    Number.isFinite(payload.summary.robustnessDiagnostics?.historyDepthScore),
  );
  assert.ok(
    Number.isFinite(
      payload.opportunityDiscovery?.diagnostics?.regimeCoverageScore,
    ),
  );
  ...
  assert.ok(
    payload.opportunityDiscovery?.candidates?.length > 0,
    "opportunity discovery should rank emerging candidates",
  );
  assert.ok(
    payload.opportunityDiscovery?.density?.density >= 0,
    "adaptive opportunity density should be present",
  );
  assert.ok(
    payload.agencyDiagnostics?.summary?.traceCount > 0,
    "agency diagnostics should trace strategy decisions",
  );
  assert.equal(
    payload.agencyDiagnostics.summary.traceCount,
    payload.signals.length,
  );
  assert.ok(
    payload.recognitionDiagnostics?.primary,
    "recognition diagnostics should summarize recurrence evidence",
  );
  assert.equal(
    payload.summary.recognitionDiagnostics?.signals?.length,
    payload.signals.length,
  );
  assert.ok(
    payload.signals.every(
      (signal: any) => signal.recognition?.metadata?.module === "recognition",
    ),
    "signals should carry Recognition diagnostics",
  );
  assert.ok(
    payload.signals.every((signal: any) => signal.agencyTrace?.traceId),
    "signals should carry agency traces",
  );
  assert.ok(
    payload.signals.some(
      (signal: any) => signal.survivalMemory?.recordCount > 0,
    ),
    "signals should carry survival memory diagnostics from outcome history",
  );
  assert.ok(
    payload.signals.some(
      (signal: any) => signal.opportunityDiscovery?.progression?.length > 0,
    ),
...
test("anti-overfit gates separate historical robustness from promotion evidence", async () => {
  const cacheDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "signal-market-backtest-robust-"),
  );
  process.env.LOCAL_BACKTEST_CACHE_DIR = cacheDir;
  ...
  assert.ok(
    payload.summary.maxDrawdownPct > 0.5,
    "robust profile should include meaningful drawdown",
  );
  assert.ok(
    payload.summary.profitFactor < 20,
    "profit factor should stay below the overfit threshold",
  );
  ...
  assert.ok(
    selectedCandidate,
    "health optimizer should expose the selected BINANCE candidate first",
  );
  assert.ok(
    candidates.every(
      (candidate: any) =>
        candidate.readinessGateCount <= selectedCandidate.readinessGateCount,
    ),
    "selection should prioritize readiness gates before cosmetic concentration cleanliness",
  );
  assert.equal(
    typeof selectedCandidate.selectionGates.benchmarkPass,
    "boolean",
  );
  assert.equal(
    typeof selectedCandidate.selectionGates.parameterPass,
    "boolean",
  );
...
test("strategy health objective prefers robust risk-adjusted edge over raw return", () => {
  ...
  assert.ok(
    robust > fragile,
    "health selection should reward Sharpe, drawdown, and distributed returns",
  );
  assert.ok(
    underSampled < robust,
    "thin samples should not win the health objective",
  );
});
...
test("health optimized candidates include protective variants without mutating the base config", () => {
  ...
  assert.ok(
    candidates.some((candidate) => candidate.id === config.id),
    "base candidate should remain available",
  );
  assert.ok(
    candidates.some((candidate) =>
      candidate.id.includes("crypto-low-drawdown"),
    ),
    "crypto profile should include drawdown defense",
  );
  assert.ok(
    candidates.some((candidate) =>
      candidate.id.includes("crypto-distributed-survival"),
    ),
    "crypto profile should include distributed survival confirmation",
  );
  assert.ok(
    candidates.some((candidate) =>
      candidate.id.includes("crypto-benchmark-balanced"),
    ),
    "crypto profile should include benchmark-balanced confirmation",
  );
  assert.ok(
    candidates.some(
      (candidate) => candidate.maxPositionPct < config.maxPositionPct,
    ),
    "nearby variants should reduce concentration risk",
  );
  assert.ok(
    candidates.every(
      (candidate) => candidate.stopLossPct > 0 && candidate.trailingStopPct > 0,
    ),
  );
});
...
test("B3 health candidates include concentration-reduction variants", () => {
  ...
  assert.ok(
    candidates.some((candidate) =>
      candidate.id.includes("b3-distributed-quality"),
    ),
  );
  assert.ok(
    candidates.some((candidate) => candidate.id.includes("b3-median-return")),
  );
  assert.ok(
    candidates.some(
      (candidate) =>
        candidate.maxPositions > config.maxPositions &&
        candidate.maxPositionPct < config.maxPositionPct,
    ),
  );
});
...
test("diagnostic raw technical mode emits explainable trades and survival analytics", async () => {
  const cacheDir = fs.mkdtempSync(
    path.join(os.tmpdir(), "signal-market-backtest-diagnostics-"),
  );
  process.env.LOCAL_BACKTEST_CACHE_DIR = cacheDir;
  ...
  const buySignals = payload.signals.filter(
    (signal: any) => signal.signalAction === "Buy",
  );
  const sellSignals = payload.signals.filter(
    (signal: any) => signal.signalAction === "Sell",
  );
  const rawBuyCandidates = payload.signals.filter(
    (signal: any) => signal.diagnostic?.rawAction === "Buy",
  );
  const blockedSignals = payload.signals.filter(
    (signal: any) => signal.allocationAction === "Blocked",
  );
  ...
  assert.ok(
    payload.trades.length > 0,
    "raw technical validation should create trades",
  );
  assert.ok(
    rawBuyCandidates.length > 0,
    "raw technical validation should expose buy candidates in diagnostics",
  );
  assert.ok(
    sellSignals.length > 0,
    "raw technical validation should expose sell signals",
  );
  if (payload.summary.strategyReadiness?.blocked) {
    assert.equal(
      buySignals.length,
      0,
      "blocked readiness should not leak buy allocations",
    );
    assert.ok(
      blockedSignals.length > 0,
      "blocked readiness should label zero-position assets as blocked",
    );
    assert.ok(
      blockedSignals.every((signal: any) => signal.suggestedExposure === 0),
    );
  } else {
    assert.ok(
      buySignals.length > 0,
      "passed readiness should expose buy allocations",
    );
    assert.ok(
      buySignals.every((signal: any) => signal.entryPrice > 0),
      "buy signals should carry shadow entry prices",
    );
  }
  assert.ok(
    rawBuyCandidates.every((signal: any) => signal.signalDate),
    "raw buy candidates should carry a shadow signal date",
  );
  ...
  assert.ok(
    payload.diagnostics.auditTrail.length > 0,
    "diagnostics should include audit events",
  );
  assert.ok(
    finalDecisionStage?.passed > 0,
    "assets should survive to final decision",
  );
  assert.ok(
    payload.diagnostics.modeComparison.some(
      (row: any) => row.mode === "MODE_FULL_PERCEPTION",
    ),
