...
export function evaluatePortfolioRisk(input: PortfolioRiskInput): PortfolioRiskResult {
  ...
  const exposureByAsset: Record<string, number> = { ...input.currentExposureByAsset };
  ...
  for (const signal of input.signals) {
    ...
    if (decision.accepted) {
      ...
      exposureByAsset[assetKey] = (exposureByAsset[assetKey] ?? 0) + decision.position.position.size;
...
function evaluateSignal(
  ...
  if (signal.confidence < ctx.config.minConfidence) {
    rejectionReasons.push(`confidence_below_threshold:${signal.confidence.toFixed(3)}<${ctx.config.minConfidence}`);
  }

  // Minimum strength check
  if (signal.strength < ctx.config.minStrength) {
    rejectionReasons.push(`strength_below_threshold:${signal.strength.toFixed(3)}<${ctx.config.minStrength}`);
  }
  ...
  const rawSize = ctx.config.baseSizeFraction * signal.confidence * signal.strength * ctx.equity;
  ...
  const remainingAssetCapacity = Math.max(0, maxAssetExposure - currentAssetExposure);
  ...
  const remainingTotalCapacity = Math.max(0, maxTotalExposure - ctx.totalExposure);
  ...
  const position: Position = {
    asset: signal.asset,
    size: positionSize,
    direction: signal.direction === "long" ? "long" : "short" as PositionDirection,
  };

  reasons.push(`sized_from_confidence_${signal.confidence.toFixed(3)}_strength_${signal.strength.toFixed(3)}`);
  if (positionSize < rawSize) {
    reasons.push(`size_reduced_from_${round8(rawSize)}_to_${positionSize}_due_to_limits`);
  }
...
function resolveConfig(partial?: Partial<PortfolioRiskConfig>): PortfolioRiskConfig {
  if (!partial) return DEFAULT_PORTFOLIO_RISK_CONFIG;
  return {
    maxExposurePerAsset: partial.maxExposurePerAsset ?? DEFAULT_PORTFOLIO_RISK_CONFIG.maxExposurePerAsset,
    maxTotalExposure: partial.maxTotalExposure ?? DEFAULT_PORTFOLIO_RISK_CONFIG.maxTotalExposure,
    minConfidence: partial.minConfidence ?? DEFAULT_PORTFOLIO_RISK_CONFIG.minConfidence,
    minStrength: partial.minStrength ?? DEFAULT_PORTFOLIO_RISK_CONFIG.minStrength,
    cooldownMs: partial.cooldownMs ?? DEFAULT_PORTFOLIO_RISK_CONFIG.cooldownMs,
    baseSizeFraction: partial.baseSizeFraction ?? DEFAULT_PORTFOLIO_RISK_CONFIG.baseSizeFraction,
  };
}
