...
export function parseLimit(value: unknown, fallback: number) {
  const normalized = String(value ?? "")
    ...
    .toLowerCase();
  if (
    ["auto", "system", "unlimited", "infinite", "infinity", "none"].includes(
      normalized,
    )
  ) {
...
export function parseAllocationMode(
  value: unknown,
): BinanceExecutionConfig["allocationMode"] {
  const normalized = String(value ?? "")
    .trim()
    .toLowerCase();
  return normalized === "system_proportional" ||
    normalized === "system" ||
...
export function modeFromEnv(value: unknown): ExecutionMode {
  const normalized = String(value ?? "dry_run")
    .trim()
    .toLowerCase();
...
export function defaultExecutionStateFile(
  env: NodeJS.ProcessEnv = process.env,
) {
  if (env.VERCEL) return path.join(os.tmpdir(), "binance-execution-state.json");
  return path.resolve(
    process.cwd(),
    ".local-cache/binance-execution-state.json",
  );
}
...
export function loadBinanceExecutionConfig(
  ...
  const allowedSymbols =
    input.allowedSymbols ?? parseAllowedSymbols(env.BINANCE_ALLOWED_SYMBOLS);
  ...
  const config: BinanceExecutionConfig = {
    ...
    testnetBaseUrl:
      input.testnetBaseUrl ??
      env.BINANCE_TESTNET_BASE_URL ??
      "https://testnet.binance.vision",
    allowedSymbols,
    maxNotionalPerOrder:
      input.maxNotionalPerOrder ??
      parseLimit(env.BINANCE_MAX_NOTIONAL_PER_ORDER, 100),
    maxDailyNotional:
      input.maxDailyNotional ?? parseLimit(env.BINANCE_MAX_DAILY_NOTIONAL, 500),
    maxOpenOrders:
      input.maxOpenOrders ?? parseLimit(env.BINANCE_MAX_OPEN_ORDERS, 5),
    allocationMode:
      input.allocationMode ?? parseAllocationMode(env.BINANCE_ALLOCATION_MODE),
    minConfidence:
      input.minConfidence ?? parseNumber(env.BINANCE_MIN_CONFIDENCE, 0.55),
    minTrust: input.minTrust ?? parseNumber(env.BINANCE_MIN_TRUST, 0.55),
    strategyEquityCap:
      input.strategyEquityCap ??
      parseNumber(env.BINANCE_STRATEGY_EQUITY_CAP, Number.POSITIVE_INFINITY),
    accountEquityOverride:
      input.accountEquityOverride ??
      parseNumber(env.BINANCE_ACCOUNT_EQUITY_OVERRIDE, 20),
    staleDecisionMs:
      input.staleDecisionMs ??
      parseNumber(env.BINANCE_STALE_DECISION_MS, 5 * 60_000),
    staleSyncMs:
      input.staleSyncMs ?? parseNumber(env.BINANCE_STALE_SYNC_MS, 2 * 60_000),
    cooldownMs:
      input.cooldownMs ?? parseNumber(env.BINANCE_DECISION_COOLDOWN_MS, 15_000),
    recvWindow:
      input.recvWindow ?? parseNumber(env.BINANCE_RECV_WINDOW_MS, 5_000),
    allowMarketOrders:
      input.allowMarketOrders ?? parseBoolean(env.ALLOW_MARKET_ORDERS, false),
    confirmLiveTrading:
      input.confirmLiveTrading ??
      parseBoolean(env.BINANCE_CONFIRM_LIVE_TRADING, false),
    liveTradingEnabled:
      input.liveTradingEnabled ??
      parseBoolean(env.BINANCE_LIVE_TRADING_ENABLED, false),
    riskGuard: {
      liveTradingApproved:
    ...
    exchangeInfoTtlMs:
      input.exchangeInfoTtlMs ??
      parseNumber(env.BINANCE_EXCHANGE_INFO_TTL_MS, 30 * 60_000),
    requestTimeoutMs:
      input.requestTimeoutMs ??
      parseNumber(env.BINANCE_REQUEST_TIMEOUT_MS, 10_000),
    fetch: fetchImpl,
    validationErrors: [],
  };
...
export function validateBinanceExecutionConfig(config: BinanceExecutionConfig) {
  ...
  if (config.mode === "live") {
    if (config.liveTradingEnabled !== true)
      errors.push("live_trading_env_disabled");
    if (config.confirmLiveTrading !== true)
      errors.push("live_trading_not_confirmed");
    if (config.riskGuard.liveTradingApproved !== true)
      errors.push("live_trading_not_approved");
  }
