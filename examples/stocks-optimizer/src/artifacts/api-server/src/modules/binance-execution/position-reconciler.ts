...
export class PositionReconciler {
  ...
  reconcile(account: AccountState = this.store.getAccountState()) {
    ...
    const actualPositions = actualPositionsFromBalances(account, records.orders);
    ...
    if (account.openOrders.length > records.orders.filter((order) => ["NEW", "PARTIALLY_FILLED"].includes(order.status.toUpperCase())).length + 5) {
...
function expectedPositionsFromOrders(orders: ExecutionOrderRecord[]) {
  ...
  for (const order of orders) {
    ...
    positions[order.symbol] = (positions[order.symbol] ?? 0) + (order.side === "BUY" ? filledQuantity : -filledQuantity);
  }
  return positions;
}

function actualPositionsFromBalances(account: AccountState, orders: ExecutionOrderRecord[]) {
  const positions: Record<string, number> = {};
...
function exposureFromOrders(orders: ExecutionOrderRecord[]) {
  return orders
    .filter((order) => ["NEW", "PARTIALLY_FILLED", "FILLED"].includes(order.status.toUpperCase()))
