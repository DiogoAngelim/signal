...
export function computeAvailableStrategyEquity(input: {
  ...
  const equity = input.availableEquity == null
    ? Math.max(0, finite(input.accountEquity, 0) - Math.max(0, finite(input.reservedCapital, 0)))
    : Math.max(0, finite(input.availableEquity, input.accountEquity));
  const cap = Math.max(0, finite(input.strategyEquityCap, Number.POSITIVE_INFINITY));
  return Math.max(0, Math.min(equity, cap));
}

export function allocateProportionalNotional(input: AllocationInput): AllocationResult[] {
  const buys = input.decisions.filter((decision) => decision.action === "BUY");
  const weights = buys.map((decision) => normalizeAppSizeToWeight(decision.appSizePct));
  const totalWeight = weights.reduce((sum, weight) => sum + weight, 0);
  const remainingDaily = Math.max(0, input.maxDailyNotional - (input.usedDailyNotional ?? 0));
  ...
  return input.decisions.map((decision) => {
    ...
    if (decision.action === "BUY" && weight <= 0) reasons.push("non_positive_weight");
    if (decision.action === "BUY" && totalWeight <= 0) reasons.push("no_positive_weights");

    const denominator = input.useFullAvailableEquity
      ? Math.max(totalWeight, Number.EPSILON)
      : totalWeight > 1 ? totalWeight : 1;
    const proportional = decision.action === "BUY" && totalWeight > 0 ? (weight / denominator) * allocatable : 0;
    const hasSuggested = Number.isFinite(decision.suggestedNotional);
    const suggested = hasSuggested ? Number(decision.suggestedNotional) : proportional;
    const notional = decision.action === "SELL" || decision.action === "EXIT"
      ? 0
      : Math.max(0, Math.min(proportional, suggested, input.maxOrderNotional));
    const minNotional = input.minNotionalBySymbol?.[decision.symbol] ?? 0;

    if (notional > input.maxOrderNotional) reasons.push("max_order_notional_exceeded");
    ...
    if (notional <= 0 && decision.action === "BUY") reasons.push("zero_allocation");
