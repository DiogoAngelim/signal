...
export class DecisionMemoryContractAdapter {
  ...
  async recordDecision(
    input: DecisionRecordContractInput,
    ...
    if (existing) {
      throw new MemoryAppendOnlyError(
        `Decision memory is append-only for ${scopeKey(scope)}.`,
      );
    }
    ...
    const record = createDecisionRecord({
      ...
      source,
      createdAt,
      observation: scopedPayload(
        input.observation ?? input.record?.observation ?? {},
        governance,
      ),
      realitySnapshotId:
        input.record?.realitySnapshotId ?? `reality:${storageDecisionId}`,
      realitySnapshot: input.record?.realitySnapshot
        ? {
            ...input.record.realitySnapshot,
            snapshotId:
              input.record.realitySnapshot.snapshotId ??
              `reality:${storageDecisionId}`,
            source,
            createdAt,
            metadata: scopedMetadata(
              input.record.realitySnapshot.metadata,
              governance,
            ),
          }
        : undefined,
      ...
      retentionTier:
        input.retentionTier ?? input.record?.retentionTier ?? "hot",
    });
    const saved = await this.store.saveDecisionRecord(record);
    await this.store.saveRegimeSnapshot(
      regimeSnapshotFromDecision(saved, scope, governance),
    );
    return saved;
  }

  async recordOutcome(
    input: OutcomeRecordContractInput,
  ...
  async recordReview(
    input: ReviewRecordContractInput,
    ...
    const review: DecisionReview = {
      ...
      reviewId:
        ...
        `review:${storageDecisionId}:${scope.timestamp}`,
      decisionId: storageDecisionId,
      appId: scope.appId,
      domain: scope.domain,
      timestamp: scope.timestamp,
      ...
      classification:
        input.classification ?? input.review?.classification ?? "inconclusive",
      whatWasRecommended:
        input.whatWasRecommended ??
        input.review?.whatWasRecommended ??
        "Unknown recommendation.",
      whyRecommended:
        input.whyRecommended ??
        input.review?.whyRecommended ??
        "No rationale recorded.",
      whatHappened:
        input.whatHappened ??
        input.review?.whatHappened ??
        "Outcome review was inconclusive.",
      lesson:
        input.lesson ??
        input.review?.lesson ??
        "Keep the lesson provisional until more reviewed outcomes arrive.",
      confidenceAdjustment:
        input.confidenceAdjustment ?? input.review?.confidenceAdjustment ?? 0,
      trustAdjustment:
        input.trustAdjustment ?? input.review?.trustAdjustment ?? 0,
  ...
  async recordLesson(
    input: LessonRecordContractInput,
  ...
  async querySimilarity(
    input: SimilarityQueryContractInput,
  ): Promise<SimilarityQueryContractResult> {
    const scope = assertMemoryScope(input.scope);
    const current = input.current ?? (await this.currentRegime(scope));
    const history = (
      await this.store.listRegimeSnapshots({
        appId: scope.appId,
        domain: scope.domain,
        source: scope.appId,
        limit: 1_000,
    ...
    for (const match of matches) {
      const matchedScope = memoryScopeFromRecord(match.snapshot) ?? {
        ...scope,
        decisionId:
          match.snapshot.decisionId ?? match.snapshot.regimeSnapshotId,
      };
      ...
      const classification =
        match.snapshot.eventualOutcome?.classification ?? "unknown";
      outcomeDistribution[classification] =
        (outcomeDistribution[classification] ?? 0) + 1;
      similarCases.push({
  ...
  async queryCalibration(
    input: CalibrationQueryContractInput,
  ): Promise<CalibrationQueryContractResult> {
    const scope = assertMemoryScope(input.scope);
    const storageDecisionId = memoryStorageDecisionId(scope);
    const records = (
      await this.store.listCalibrationRecords({
        appId: scope.appId,
        domain: scope.domain,
        decisionId: storageDecisionId,
    ...
    return {
      scope,
      confidenceAccuracy:
        latest?.actualOutcomeScore == null
          ? 0
          : Math.max(
              0,
              Math.round(
                100 -
                  Math.abs(
                    latest.predictedConfidence - latest.actualOutcomeScore,
                  ),
              ),
            ),
  ...
  async timeline(
    input: MemoryTimelineContractInput,
  ): Promise<MemoryTimelineContractResult> {
    const scope = assertMemoryScope(input.scope);
    const storageDecisionId = memoryStorageDecisionId(scope);
    const decision =
      (await this.store.getDecisionRecord(storageDecisionId)) ?? null;
    const outcomes = await this.store.listOutcomes(storageDecisionId);
    const reviews = (
      await this.store.listDecisionReviews({
        appId: scope.appId,
        domain: scope.domain,
        decisionId: storageDecisionId,
        limit: 1_000,
    ...
    const lessons = (
      await this.store.listLearningRecords({
        appId: scope.appId,
        domain: scope.domain,
        decisionId: storageDecisionId,
        limit: 1_000,
    ...
    const entries = [
      ...
      ...outcomes.map((outcome) =>
        timelineEntry(
          "Outcome",
          outcome.outcomeId,
          outcome.timestamp ?? scope.timestamp,
          outcome,
        ),
      ),
      ...reviews.map((review) =>
        timelineEntry("Review", review.reviewId, review.reviewedAt, review),
      ),
      ...lessons.map((lesson) =>
        timelineEntry("Lesson", lesson.learningId, lesson.createdAt, lesson),
      ),
  ...
  async stats(
    ...
    const outcomes = (await this.store.listOutcomes()).filter(
      (outcome) => outcome.appId === appId && outcome.domain === domain,
    );
    const reviews = (
      await this.store.listDecisionReviews({ appId, domain, limit: 1_000 })
    ).filter((review) => matchesMemoryScope(review, { appId, domain }));
    const lessons = (
      await this.store.listLearningRecords({ appId, domain, limit: 1_000 })
    ).filter((lesson) => matchesMemoryScope(lesson, { appId, domain }));
    const calibrationRecords = (
      await this.store.listCalibrationRecords({ appId, domain, limit: 1_000 })
    ).filter((record) => matchesMemoryScope(record, { appId, domain }));
    const replaySnapshots = (
      await Promise.all(
        decisions.map((decision) =>
          this.store.listReplaySnapshots(decision.decisionId),
        ),
      )
    )
      .flat()
      .filter(
        (snapshot) => snapshot.appId === appId && snapshot.domain === domain,
      );
  ...
  private async requireDecision(storageDecisionId: string, scope: MemoryScope) {
    ...
    if (!decision) {
      throw new MemoryAppendOnlyError(
        `Cannot append memory to unknown decision ${scopeKey(scope)}.`,
      );
    }
    return decision;
  }
  ...
  private async assertNoOutcome(outcomeId: string) {
    const existing = (await this.store.listOutcomes()).find(
      (outcome) => outcome.outcomeId === outcomeId,
    );
    if (existing)
      throw new MemoryAppendOnlyError(`Outcome ${outcomeId} already exists.`);
  }

  private async assertNoReview(reviewId: string) {
    const existing = (
      await this.store.listDecisionReviews({ limit: 1_000 })
    ).find((review) => review.reviewId === reviewId);
    if (existing)
      throw new MemoryAppendOnlyError(`Review ${reviewId} already exists.`);
  }

  private async assertNoLesson(learningId: string) {
    const existing = (
      await this.store.listLearningRecords({ limit: 1_000 })
    ).find((lesson) => lesson.learningId === learningId);
    if (existing)
      throw new MemoryAppendOnlyError(`Lesson ${learningId} already exists.`);
  }
  ...
  private async currentRegime(scope: MemoryScope): Promise<RegimeSnapshot> {
    ...
    return (
      snapshots[0] ??
      regimeSnapshotFromDecision(
        await this.requireDecision(storageDecisionId, scope),
        scope,
        governanceFor(scope, undefined, "Similarity"),
      )
    );
  }

  private async saveCalibrationForOutcome(
    ...
    const calibrationError =
      Math.round((predictedConfidence - actualOutcomeScore) * 100) / 100;
    ...
    const record: CalibrationRecord = {
      ...
      calibrationScore: Math.max(
        0,
        Math.round(100 - Math.abs(calibrationError)),
      ),
  ...
  private async attachOutcomeToRegime(
    outcome: OutcomeEvaluation,
    scope: MemoryScope,
    governance: MemoryRecordGovernance,
  ) {
    ...
    await this.store.saveRegimeSnapshot({
      ...
      eventualOutcome: {
        classification:
          ...
          outcome.category === "unexpected-success"
            ? "correct"
            : outcome.category === "failure" ||
                outcome.category === "unexpected-failure"
              ? "wrong"
              : "inconclusive",
        summary:
          outcome.lessons[0] ?? `Outcome recorded as ${outcome.category}.`,
...
export function createDecisionMemoryContractAdapter(
...
export function memoryScopeFromRecord(
  record: unknown,
  ...
  if (observation && typeof observation === "object") {
    const observedMetadata = metadataFrom(
      observation as Record<string, unknown>,
    );
    const observedScope = observedMetadata
      ? scopeFromFields(observedMetadata.scope)
      : undefined;
...
export function matchesMemoryScope(
  record: unknown,
  filter: { appId?: string; domain?: string; decisionId?: string },
): boolean {
  ...
  if (
    filter.decisionId &&
    memoryStorageDecisionId(scope) !== filter.decisionId &&
    scope.decisionId !== filter.decisionId
  )
    return false;
  return true;
}
...
function regimeSnapshotFromDecision(
  ...
  return {
    ...
    trust: Math.max(
      0,
      Math.min(100, record.coherence.score + record.coherence.trustAdjustment),
    ),
    confidence: record.coherence.score,
    readiness: record.coherence.actionAllowed
      ? Math.max(50, record.coherence.actionScale * 100)
      : Math.min(49, record.coherence.actionScale * 100),
...
function scopeOutcome(
  ...
  return {
    ...outcome,
    outcomeId:
      outcome.outcomeId ?? `outcome:${storageDecisionId}:${scope.timestamp}`,
    decisionId: storageDecisionId,
    originalDecisionId: scope.decisionId,
    appId: scope.appId,
    domain: scope.domain,
...
function governanceFor(
  ...
  recordKind: MemoryRecordGovernance["recordKind"],
): MemoryRecordGovernance {
  return {
    scope,
    correlationId:
      correlationId ??
      `corr:${memoryStorageDecisionId(scope)}:${recordKind}:${scope.timestamp}`,
...
function scopeFromFields(value: unknown): MemoryScope | undefined {
  ...
  const decisionId = stringOrUndefined(
    object.originalDecisionId ?? object.decisionId,
  );
  const timestamp = stringOrUndefined(
    object.timestamp ??
      object.createdAt ??
      object.reviewedAt ??
      object.recordedAt,
...
function actionLabel(action: unknown): string {
  ...
  if (action && typeof action === "object") {
    const value =
      ...
      (action as Record<string, unknown>).recommendation;
...
function requiredText(value: unknown, label: string): string {
  ...
  if (!text)
    throw new MemoryScopeError(`${label} is required for memory scope.`);
  return text;
}
