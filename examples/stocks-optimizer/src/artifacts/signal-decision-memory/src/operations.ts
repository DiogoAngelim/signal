...
export const DECISION_MEMORY_OPERATION_DEFINITIONS = [
  ...
  operation(
    "mutation",
    "outcome.record.v1",
    "Record a scoped durable decision outcome.",
    true,
  ),
  operation(
    "mutation",
    "review.record.v1",
    "Record a scoped durable decision review.",
    true,
  ),
  operation(
    "mutation",
    "lesson.record.v1",
    "Record a scoped durable decision lesson.",
    true,
  ),
  operation(
    "query",
    "similarity.query.v1",
    "Query scoped similar cases and outcome distribution.",
    true,
  ),
  operation(
    "query",
    "calibration.query.v1",
    "Query scoped calibration history.",
    true,
  ),
  operation(
    "query",
    "memory.timeline.v1",
    "Reconstruct a scoped decision timeline.",
    true,
  ),
...
export function registerDecisionMemoryOperations(input: {
  ...
  for (const definition of operations) {
    ...
    if (definition.kind === "mutation")
      input.registry.registerMutation?.(definition);
  ...
  return [
        ...
        const saved = await store.saveRealitySnapshot({
          ...
          metadata: {
            ...
            [MEMORY_SCOPE_METADATA_KEY]: {
              scope,
              correlationId:
                stringOrUndefined(input.correlationId) ??
                `corr:reality:${snapshot.snapshotId}`,
              version: "v1",
              recordKind: "Decision",
            },
          },
    ...
    },
    {
      ...
      async handler(
        ...
        const saved = await contracts.recordDecision(
          input as DecisionRecordContractInput,
        );
    ...
    },
    {
      ...
      async handler(input: Record<string, unknown>) {
        ...
        const records = await store.listDecisionRecords({
          ...
          domain: scope.domain,
          source: scope.appId,
          retentionTier: input.retentionTier as
            | "hot"
            | "warm"
            | "cold"
            | "expired"
    ...
    },
    {
      ...
      async handler(
        ...
        const saved = await contracts.recordOutcome(
          input as OutcomeRecordContractInput,
        );
      ...
      normalizeIdempotencyInput(input: Record<string, unknown>) {
        return {
          scope: input.scope,
          outcomeId: (input.outcome as Record<string, unknown> | undefined)
            ?.outcomeId,
        };
      },
    },
    {
      ...
      idempotency: "optional" as const,
      async handler(input: Record<string, unknown>) {
        const review = await contracts.recordReview(
          input as ReviewRecordContractInput,
        );
      ...
      normalizeIdempotencyInput(input: Record<string, unknown>) {
        return {
          scope: input.scope,
          reviewId: (input.review as Record<string, unknown> | undefined)
            ?.reviewId,
        };
      },
    },
    {
      ...
      idempotency: "optional" as const,
      async handler(input: Record<string, unknown>) {
        const lesson = await contracts.recordLesson(
          input as LessonRecordContractInput,
        );
    ...
    },
    {
      ...
      resultSchema: z.record(z.string(), z.unknown()),
      async handler(input: Record<string, unknown>) {
        return contracts.queryCalibration(
          input as Parameters<typeof contracts.queryCalibration>[0],
        );
      },
    },
    {
      ...
      resultSchema: z.record(z.string(), z.unknown()),
      async handler(input: Record<string, unknown>) {
        return contracts.timeline(
          input as Parameters<typeof contracts.timeline>[0],
        );
      },
    },
    {
      ...
      async handler(input: Record<string, unknown>) {
        ...
        if (!record)
          return {
            ...
            explanation: "Decision was not found.",
          };
        const current = currentCoherenceFrom(input);
        const replay = current
          ? compareReplay(record.decisionId, record.coherence, current)
          : undefined;
    ...
    },
    {
      ...
      async handler(
        ...
        const source = String(
          input.source ?? process.env.SIGNAL_SOURCE_ID ?? "signal",
        );
        ...
        const trust: TrustHistoryEntry = {
          trustId: String(
            input.trustId ?? `trust:${decisionId ?? "global"}:${Date.now()}`,
          ),
          ...(decisionId ? { decisionId } : {}),
          source,
        ...
        await context.emit?.("decision.calibration_updated.v1", {
          decisionId,
          source,
        });
        return {
    ...
    ...DECISION_MEMORY_OPERATION_DEFINITIONS.filter(
      (definition) => definition.kind === "event",
    ).map((definition) => ({
...
function recordFromInput(input: Record<string, unknown>): SignalDecisionRecord {
  ...
  return createDecisionRecord({
    ...
    realitySnapshot: objectOrUndefined(input.realitySnapshot) as
      | RealitySnapshot
      | undefined,
    ...
    need: input.need,
    coherence:
      (input.coherence as SignalDecisionRecord["coherence"] | undefined) ??
      assessCoherence(moduleInputs(input.modules)),
...
function realitySnapshotFromInput(
  ...
  return createRealitySnapshot({
    ...
    source:
      stringOrUndefined(input.source) ??
      process.env.SIGNAL_SOURCE_ID ??
      "signal",
    ...
    sourceRef: objectOrUndefined(
      input.sourceRef,
    ) as RealitySnapshot["sourceRef"],
    metadata: objectOrUndefined(input.metadata) as
...
function outcomeFromInput(input: Record<string, unknown>): OutcomeEvaluation {
  ...
  return evaluateOutcome({
    ...
    lessons: Array.isArray(input.lessons)
      ? input.lessons.map((lesson) => String(lesson))
      : undefined,
  });
}

function currentCoherenceFrom(
  input: Record<string, unknown>,
): CoherenceAssessment | undefined {
