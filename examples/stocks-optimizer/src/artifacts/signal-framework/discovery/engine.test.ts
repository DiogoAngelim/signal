...
import { discover, type DiscoveryEvidence, type DiscoveryInput } from "./engine";
...
function eligibleInput(overrides: Partial<DiscoveryInput> = {}): DiscoveryInput {
  return {
    ...
    constraints: [{ id: "required-confirmation", label: "Required confirmation", passed: true, severity: "high" }],
...
test("Discovery covers defaults, weak evidence, emerging, eligible, penalties, and deterministic traces", () => {
  ...
  const malformed = discover({ state: null as unknown as Record<string, unknown> });
  const weak = discover({
  ...
  const emerging = discover(eligibleInput({
    candidates: [{ id: "emerging", label: "Emerging candidate", score: 50, confidence: 58, previousScore: 30, persistence: 54 }],
    evidence: [evidence("support-1", "Early confirmation", "support", 70, 70)],
    priorOutcomes: [],
    constraints: [{ id: "missing-independent-review", label: "Independent review", missingEvidence: "independent review" }],
  }));
  ...
  const poor = discover(eligibleInput({ priorOutcomes: [negativeOutcome("failure-1"), negativeOutcome("failure-2"), negativeOutcome("failure-3")] }));
...
test("Discovery reports contradictions, foresight, lifecycle transitions, and domain-neutral source text", () => {
  const contradiction = discover(eligibleInput({
    evidence: [
  ...
  const sized = discover(eligibleInput({ candidates: [{ id: "sized", score: 80, confidence: 80, lifecycleStatus: "sized", previousScore: 66 }] }));
  const active = discover(eligibleInput({ candidates: [{ id: "active", score: 82, confidence: 84, status: "active", previousScore: 72 }] }));
  const closed = discover(eligibleInput({ candidates: [{ id: "closed", score: 55, confidence: 55, status: "closed", previousScore: 82 }] }));
  const deterministic = discover(eligibleInput({
    ...
    constraints: [{ id: "medium", passed: false, severity: "medium" }, { id: "scored", score: 65 }],
  }));
  const source = readFileSync(new URL("./engine.ts", import.meta.url), "utf8").toLowerCase();

  assert.equal(contradiction.explanation.contradictoryEvidence[0]?.label, "Conflicting observation");
  assert.ok(contradiction.invalidationConditions.includes("The primary assumption fails."));
  assert.equal(sized.status, "sized");
  assert.equal(active.status, "active");
  assert.equal(closed.status, "closed");
  assert.deepEqual(discover(eligibleInput({
    candidates: [{ id: "deterministic", score: 62, confidence: 64, previousScore: 62, persistence: 62 }],
    evidence: [
      evidence("neutral", "Neutral evidence", "neutral", 50, 50),
      { id: "negative-score", label: "Negative raw score", strength: -5 },
  ...
  for (const word of ["sharpe", "portfolio", "binance", "crypto", "drawdown", "exposure", "ticker"]) {
...
test("Discovery covers aggregate, fallback, scoped evidence, and next-step paths", () => {
  const aggregate = discover({
    ...
    evidence: [
      { description: "Description-only support", source: "sensor", strength: 96, confidence: 92, weight: 0, predictive: true },
    ...
    priorOutcomes: [
      { candidateId: "aggregate-subject", state: { demand: "high", ready: true }, result: "valid", evidence: [{ id: "Description-only support" }] },
      { state: { demand: "high", ready: true }, outcome: "unknown", evidence: [{}] },
      { outcome: "neutral", evidence: [{ label: "No state evidence" }] },
    ],
    constraints: [{ id: "default-severity", passed: false, missingEvidence: "operator review" }],
  ...
  const fragile = discover(eligibleInput({
    ...
    evidence: [evidence("support-1", "Support remains", "support", 100, 100), evidence("contradict-1", "Major contradiction", "contradict", 79, 79)],
    historicalStates: [],
    priorOutcomes: [],
  ...
  const oneMissing = discover(eligibleInput({
    ...
    evidence: [evidence("support-1", "Scoped elsewhere", "support", 68, 68, "scope")].map((item) => ({ ...item, candidateId: "other-candidate" })),
    priorOutcomes: [],
  ...
  assert.deepEqual(fallbackCandidateFields.opportunities.map((item) => item.id), [
    "candidate-id-only",
    "subject-id-only",
    "aaa-tie",
    "candidate:3",
  ]);
  assert.equal(anonymousAggregate.opportunities[0]?.label, "Aggregate opportunity");
  assert.ok(oneMissing.lifecycle.transitionReason.includes("1 missing evidence item."));
  assert.equal((oneMissing.opportunities[0]?.supportingEvidence[0] as any)?.candidateId, "other-candidate");
});
