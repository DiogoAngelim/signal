...
test("Recognition reports conflicted Discovery and Judgement definitions", () => {
  ...
  assert.equal(result.discoveryNoveltyJustified, true);
  assert.equal(result.judgementSimilarityJustified, false);
  assert.ok(result.missingEvidence.includes("state-level evidence explaining Judgement similarity"));
...
test("Recognition reports insufficient evidence for sparse memory", () => {
  ...
  assert.ok(result.missingEvidence.includes("recurring state matches above the recognition threshold"));
  assert.ok(result.missingEvidence.includes("historical outcome linkage"));
  assert.ok(result.missingEvidence.includes("memory depth sufficient to justify novelty"));
});
...
test("Recognition rejects broad Judgement matches with low coverage", () => {
  ...
  assert.equal(result.matchedSamples, 0);
  assert.equal(result.judgementSimilarityJustified, false);
  assert.ok(result.invalidationConditions.includes("Invalidate Judgement similarity if feature coverage remains too broad or outcome linkage is missing."));
});
...
test("Recognition keeps fingerprints deterministic and normalizes defensive inputs", () => {
  ...
  assert.ok(malformed.invalidationConditions.includes("survival state changes"));
});
