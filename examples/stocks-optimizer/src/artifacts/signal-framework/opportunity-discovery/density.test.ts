...
test("classifies improving, weakening, and unchanged density trends", () => {
  ...
  const improving = evaluateOpportunityDensity({
    candidates,
    previousDensity: current.density - 10,
  });
  const weakening = evaluateOpportunityDensity({
    candidates,
    previousDensity: current.density + 10,
  });
  const flat = evaluateOpportunityDensity({
    candidates,
    previousDensity: current.density + 1,
  });
