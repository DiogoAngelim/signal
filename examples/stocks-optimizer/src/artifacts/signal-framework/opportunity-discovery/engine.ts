...
export function discoverOpportunities(
  ...
  for (const series of input.observationSeries ?? []) {
    const candidate = fromSeries(
      series.id,
      series.values,
      input.explorerFindings,
    );
...
function fromSeries(
  ...
  const positiveSteps = usable
    .slice(1)
    .filter((value, index) => value >= usable[index]).length;
  ...
  const strength = clamp(
    Math.abs(totalMove) * 3 +
      Math.max(0, acceleration) * 4 +
      persistence * 28 +
      Math.max(0, 18 - variability),
  );
  const evidence = [
...
function globalCandidates(
  ...
  const layerScores = Object.values(perception?.layers ?? {}).map((layer) =>
    score(layer.score, 50),
  );
  const layerMomentum = Object.values(perception?.layers ?? {}).map((layer) =>
    number(layer.momentum),
  );
  ...
  if (composite >= 62 && agreement >= 58) {
    candidates.push(
      candidate(
        "system:alignment",
        "alignment",
        mean([composite, agreement, confidence]),
        confidence,
  ...
  if ((intelligence?.contradictions ?? 0) > 0 || hasNeed(needs, "wait")) {
    candidates.push(
      candidate(
        "system:divergence",
        "divergence",
        45 + (intelligence?.contradictions ?? 0) * 8,
        confidence,
  ...
  if (
    intelligence?.transitionDetected ||
    hasNeed(needs, "discover-opportunities")
  ) {
    candidates.push(
      candidate(
        "system:transition",
        "transition",
        mean([composite, confidence, 60]),
        confidence,
...
function applyFindings(
  ...
  const matching = (findings ?? []).filter((finding) =>
    finding.feedsOpportunityTypes.includes(candidate.type),
  );
  if (!matching.length) return candidate;

  const boost = Math.min(
    12,
    mean(matching.map((finding) => finding.confidence)) * 0.08,
  );
  return {
    ...candidate,
...
function dedupe(candidates: OpportunityCandidate[]) {
  ...
  for (const candidate of candidates) {
    ...
    if (!existing || candidate.strength > existing.strength)
      best.set(candidate.opportunityId, candidate);
  }
