...
export type ExecutivePipelineOutcome =
  | "passed"
  | "limited"
  | "blocked"
  | "escalated";
...
export type DecisionCommandAction =
  | "act"
  | "wait"
  | "reduce"
  | "invalidate"
...
export type ConfidenceLedgerKind =
  | "raw"
  | "calibrated"
  | "survival"
  | "trust"
...
export type RestrictionAccountabilityStatus =
  | "pending"
  | "helpful"
  | "harmful"
  | "needs_audit";
...
export type AccountabilityStepStatus =
  | "complete"
  | "pending"
  | "review"
  | "blocked";
...
const DEFAULT_INVALIDATION =
  "Invalidate if the module evidence that justified the current decision materially weakens.";

const REASON_DEFINITIONS: Record<
  CanonicalReasonCode,
  Omit<CanonicalReason, "evidence">
> = {
  survival_scar: {
    code: "survival_scar",
    label: "Survival memory scar",
    explanation:
      "Prior similar states carried meaningful survival cost, so the system must prove recovery before normal exposure returns.",
    affectedModules: [
      "Survival Memory",
      "Recovery",
      "Sizing",
    ...
    unlockCondition:
      "Raise survival confidence to at least 70/100 or close clean reduced-size outcomes with acceptable drawdown and stress.",
    invalidationCondition:
      "Invalidate reduced-size recovery if similar-state survival cost rises or near-ruin patterns reappear.",
    priority: 10,
  },
  trust_below_threshold: {
    ...
    explanation:
      "The trust layer has not cleared the score required for normal participation.",
    affectedModules: ["Trust Governor", "Agency", "Resolve", "Calibration"],
    unlockCondition:
      "Raise trust score to at least 70/100 and clear Trust Governor blockers.",
    invalidationCondition:
  ...
  reduced_size: {
    ...
    explanation:
      "The signal may participate, but exposure is capped until safety, trust, and readiness gates mature.",
    affectedModules: ["Sizing", "Trust Governor", "Recovery", "Resolve"],
    unlockCondition:
      "Clear reduced-size review and restore normal sizing capacity.",
    invalidationCondition:
      "Invalidate normal-sizing restoration if reduced-size outcomes fail survival checks.",
    priority: 30,
  },
  recovery_incomplete: {
    ...
    explanation:
      "Recovery has not fully restored trusted capacity after prior risk or survival pressure.",
    affectedModules: [
      "Recovery",
      "Survival Memory",
      "Trust Governor",
    ...
    unlockCondition:
      "Raise recovery score and trusted capacity to the module restoration threshold.",
    invalidationCondition:
      "Invalidate recovery if drawdown, stress, or survival cost regresses.",
    priority: 40,
  },
  agency_unresolved: {
    ...
    explanation:
      "Agency has not fully approved commitment or still requires review before exposure can increase.",
    affectedModules: ["Agency", "Resolve", "Trust Governor"],
    unlockCondition:
      "Clear agency policy violations and human review requirements.",
  ...
  opportunity_density_low: {
    ...
    explanation:
      "Candidate quality or density is improving but not broad enough to justify normal exposure.",
    affectedModules: ["Discovery", "Sizing", "Opportunity"],
    unlockCondition:
      "Raise opportunity density and candidate quality above the app participation threshold.",
    invalidationCondition:
  ...
  discovery_immature: {
    ...
    explanation:
      "Discovery has not matured enough to carry larger participation without confirmation.",
    affectedModules: ["Discovery", "Recognition", "Resolve"],
    unlockCondition:
      "Raise Discovery confidence and maturity above the review threshold or let Recognition recurrence clear the review item.",
    invalidationCondition:
  ...
  calibration_review: {
    ...
    explanation:
      "Past confidence has not proven stable enough, so the dashboard should trust calibrated confidence rather than raw confidence.",
    affectedModules: ["Calibration", "Belief", "Agency", "Trust Governor"],
    unlockCondition:
      "Collect enough stable calibrated outcomes for trusted calibration status.",
    invalidationCondition:
      "Invalidate confidence expansion if calibration error widens or outcomes become unstable.",
    priority: 80,
  },
  readiness_blocked: {
    ...
    explanation:
      "Production readiness has active blockers, so the system cannot convert perception into normal live exposure.",
    affectedModules: [
      "Readiness",
      "Strategy History",
      "Trust Governor",
    ...
    unlockCondition:
      "Clear readiness blockers and reopen trusted sizing capacity.",
    invalidationCondition:
      "Invalidate live participation if readiness gates fail again.",
    priority: 90,
  },
  overfit_risk: {
    code: "overfit_risk",
    ...
    explanation:
      "Robustness checks still see enough overfit risk to limit trust in historical performance.",
    affectedModules: [
      "Judgement",
      "Robustness",
      "Strategy History",
    ...
    unlockCondition:
      "Reduce overfit risk to 30/100 or lower and preserve edge after costs.",
    invalidationCondition:
      "Invalidate if profit factor, Sharpe, or drawdown looks too clean for the sample size.",
    priority: 100,
  },
  walk_forward_instability: {
    ...
    explanation:
      "Independent test periods do not yet show enough stability for normal participation.",
    affectedModules: ["Readiness", "Strategy History", "Judgement"],
    unlockCondition:
      "Stabilize walk-forward results across at least 3 independent test periods.",
    invalidationCondition:
      "Invalidate if walk-forward returns remain concentrated or unstable.",
    priority: 110,
  },
  data_reliability_low: {
    ...
    explanation:
      "The evidence feed is not reliable enough to support larger exposure.",
    affectedModules: ["Reliability", "Readiness", "Agency", "Discovery"],
    unlockCondition:
      "Restore data reliability above 70/100 with fresh quotes, signals, and validation evidence.",
    invalidationCondition:
      "Invalidate if quote or signal coverage degrades materially.",
    priority: 120,
  },
};

export function buildExecutiveDashboardIA(
  ...
  const extracted = extractUnlockInvalidationConditions(
    input,
    canonicalReasons,
  );
  ...
  const decisionChange = buildDecisionChangeMap(
    input,
    canonicalReasons,
    extracted,
  );
  ...
  const governanceEvolution = buildGovernanceEvolution(
    input,
    canonicalReasons,
    extracted,
    decisionStates,
...
export function buildGovernanceEvolution(
  ...
  const exposureStates = buildExposureStatePlan(
    input,
    decisionStates,
    canonicalReasons,
    extracted,
  );
  const conflicts = buildGovernanceConflicts(
    input,
    canonicalReasons,
    extracted,
  );
  const vetoes = buildGovernanceVetoes(
    input,
    canonicalReasons,
    decisionStates,
    conflicts,
  );
  const command = buildGovernanceDecisionCommand(
    input,
    decisionStates,
    exposureStates,
    canonicalReasons,
  ...
  const restrictionBets = buildRestrictionAccountability(
    input,
    canonicalReasons,
  );
  const accountabilityLoop = buildAccountabilityLoop(input, restrictionBets);
  const discoveryInstitutionalization =
    buildDiscoveryInstitutionalization(input);
  const arbitration = buildGovernanceArbitration(
    input,
    command,
    vetoes,
    conflicts,
...
export function buildDecisionStates(
  input: ExecutiveDashboardInput = {},
): ExecutiveDashboardIA["decisionStates"] {
  const executiveStates = input.executive?.audit?.states;
  const computed = evaluateDecisionStates({
    confidence: firstFinite(
      input.executive?.confidence,
      input.calibration?.calibratedConfidence,
      input.calibration?.rawConfidence,
    ),
    risk: input.riskPct,
    opportunity: firstFinite(
      input.opportunity?.densityPct,
      input.opportunity?.candidateQualityPct,
    ),
    trust:
      input.decisionStates?.trust ??
      executiveStates?.trust ??
      input.executive?.trust ??
      input.trustGovernor?.trustScore,
    permission:
      input.decisionStates?.permission ??
      executiveStates?.permission ??
      input.executive?.permission,
    capacity:
      input.decisionStates?.capacity ??
      executiveStates?.capacity ??
      input.executive?.capacity ??
      input.trustGovernor?.maxExposure,
    urgency:
      input.decisionStates?.urgency ??
      executiveStates?.urgency ??
      input.executive?.urgency,
  ...
  return {
    trust:
      input.decisionStates?.trust ?? executiveStates?.trust ?? computed.trust,
    permission:
      input.decisionStates?.permission ??
      executiveStates?.permission ??
      computed.permission,
...
export function resolveCanonicalExplanations(
  input: ExecutiveDashboardInput = {},
): CanonicalReason[] {
  ...
  const add = (
    ...
    const nextEvidence = uniqueStrings([
      ...(current?.evidence ?? []),
      ...toStringArray(evidence),
    ]);
    reasons.set(code, {
      ...definition,
      evidence: nextEvidence,
      unlockCondition: firstMeaningful(
        unlock,
        current?.unlockCondition,
        definition.unlockCondition,
      ),
      invalidationCondition: firstMeaningful(
        invalidation,
        current?.invalidationCondition,
        definition.invalidationCondition,
      ),
    });
  };
  ...
  if (["scarred", "near ruin", "near_ruin"].includes(survivalStatus)) {
    add(
      "survival_scar",
      input.survivalMemory?.mainWarnings ??
        input.survivalMemory?.reasons ??
        input.survivalMemory?.status,
  ...
  const trustScore = firstFinite(
    input.trustGovernor?.trustScore,
    input.trustGovernor?.score,
  );
  if (
    ...
    add(
      ...
      firstString(blocker?.unlockCriteria) ??
        firstString(input.trustGovernor?.unlockCriteria),
      firstString(input.trustGovernor?.contradictions) ??
        firstString(input.trustGovernor?.invalidationConditions),
    );
  }

  const sizingMode = normalized(
  ...
  if (
    ...
    (maxExposure != null && maxExposure > 0 && maxExposure < 10)
  ) {
    add(
      "reduced_size",
      [
      ...
      firstString(input.recovery?.unlockConditions) ??
        firstString(input.trustGovernor?.unlockCriteria),
      firstString(input.recovery?.invalidationConditions) ??
        firstString(input.resolve?.invalidationConditions),
    );
  }
  ...
  if (agencyRecommendation && agencyRecommendation !== "act") {
    add(
      "agency_unresolved",
      input.agency?.reasons ??
        input.agencyDiagnostics?.state?.selfDiagnosis?.reasons ??
  ...
  if (opportunityDensity != null && opportunityDensity < 45) {
    add(
      ...
      input.discoveryDensity?.explanation ??
        `Opportunity density ${Math.round(opportunityDensity)}/100.`,
    );
  }

  const discoveryConfidence = firstFinite(
    input.discovery?.confidence,
    input.discoveryDensity?.confidence,
  );
  const discoveryMaturity = firstFinite(input.discovery?.maturity);
  if (
    (discoveryConfidence != null && discoveryConfidence < 60) ||
    (discoveryMaturity != null && discoveryMaturity < 60)
  ) {
    add(
      "discovery_immature",
      input.discovery?.recommendedNextStep ?? input.discovery?.status,
      firstString(input.discovery?.unlockConditions) ??
        firstUnlockMatching(input, /discovery|recognition/i),
      firstString(input.discovery?.invalidationConditions),
    );
  }

  const calibrationStatus = normalized(
    input.calibration?.status ?? input.readiness?.calibration?.status,
  );
  if (
    [
  ...
  if (
    input.readiness?.blocked === true ||
    input.strategyReadiness?.blocked === true ||
    toStringArray(input.readiness?.failureFlags).length > 0
  ) {
    add(
      "readiness_blocked",
      input.readiness?.failureFlags ??
        input.readiness?.reason ??
        input.strategyReadiness?.reason,
  ...
  const overfitRisk = firstFinite(
    ...
    input.readiness?.components?.robustness?.score != null
      ? 100 - Number(input.readiness.components.robustness.score)
      : null,
  );
  if (overfitRisk != null && overfitRisk > 30) {
    add(
      ...
      firstUnlockMatching(input, /overfit|robustness/i),
    );
  }
  ...
  if (walkForwardScore != null && walkForwardScore < 60) {
    add(
      "walk_forward_instability",
      `Walk-forward stability ${Math.round(walkForwardScore)}/100.`,
      firstUnlockMatching(input, /walk-forward|period|segment/i),
    );
  }
  ...
  if (dataReliability != null && dataReliability < 70) {
    add(
      "data_reliability_low",
      `Data reliability ${Math.round(dataReliability)}/100.`,
      firstUnlockMatching(input, /data|quote|signal/i),
    );
  }
...
export function buildEvidenceSummary(
  ...
  const similarSamples = firstFinite(
    input.recognition?.matchedSamples,
    input.judgement?.similarSampleSize,
  );
  const positiveOutcomes = firstFinite(
    input.recognition?.matchedPositiveOutcomes,
    input.judgement?.evidence?.positiveOutcomes,
  );
  const negativeOutcomes = firstFinite(
    input.recognition?.matchedNegativeOutcomes,
    input.judgement?.evidence?.negativeOutcomes,
  );
  const neutralOutcomes = firstFinite(
    input.judgement?.evidence?.neutralOutcomes,
    input.recognition?.matchedNeutralOutcomes,
  );
  const outcomeStability = firstFinite(
    input.recognition?.outcomeStability,
    input.judgement?.outcomeStability,
  );
  const overfitRisk = firstFinite(
    input.strategyHistory?.overfitRisk,
    input.backtestSummary?.overfitRiskScore,
  ...
  const profitFactor = firstFinite(
    input.strategyHistory?.profitFactor,
    input.backtestSummary?.profitFactor,
    input.backtestSummary?.profit_factor,
  );
  const maxDrawdown = firstFinite(
    input.strategyHistory?.maxDrawdownPct,
    input.backtestSummary?.maxDrawdownPct,
    input.backtestSummary?.max_drawdown_pct,
  );
  const dataReliability = dataReliabilityScore(input);
  const calibrationTrustworthiness = firstFinite(
    input.calibration?.trustworthiness,
    input.readiness?.trustworthiness,
    input.strategyReadiness?.trustworthiness,
  );
  const walkForward = walkForwardStability(input);
  const readinessScore = firstFinite(
    input.readiness?.readinessScore,
    input.readiness?.score,
    input.strategyReadiness?.readinessScore,
    input.backtestSummary?.readinessScore,
  );
  const wisdomScore = firstFinite(input.wisdom?.wisdomScore);
  const decisionQuality = firstFinite(input.wisdom?.decisionQuality);
  const restrictionValue = firstFinite(
    input.wisdom?.counterfactuals?.restrictionValue,
  );
  const opportunityCost = firstFinite(
    input.wisdom?.opportunityEconomics?.opportunityCost,
  );
  const discoveryIntelligenceScore = firstFinite(
    input.discoveryIntelligence?.score,
  );
  ...
  return [
    evidence(
      "similar-samples",
      "Similar samples",
      countValue(similarSamples),
      "Comparable historical state count.",
      countTone(similarSamples, 30),
      10,
    ),
    evidence(
      "positive-outcomes",
      "Positive outcomes",
      countValue(positiveOutcomes),
      "Positive outcomes among comparable samples.",
      countTone(positiveOutcomes, 12),
      20,
    ),
    evidence(
      "negative-outcomes",
      "Negative outcomes",
      countValue(negativeOutcomes),
      "Negative outcomes among comparable samples.",
      inverseCountTone(negativeOutcomes, positiveOutcomes),
      30,
    ),
    evidence(
      "neutral-outcomes",
      "Neutral outcomes",
      countValue(neutralOutcomes),
      "Neutral or mixed outcomes among comparable samples.",
      "neutral",
      40,
    ),
    evidence(
      "outcome-stability",
      "Outcome stability",
      pctValue(outcomeStability),
      "Stability of outcomes in similar states.",
      scoreTone(outcomeStability),
      50,
    ),
    evidence(
      "overfit-risk",
      "Overfit risk",
      pctValue(overfitRisk),
      "Residual robustness or overfit pressure.",
      inverseScoreTone(overfitRisk, 30),
      60,
    ),
    evidence(
      "sharpe-ratio",
      "Sharpe ratio",
      decimalValue(sharpeRatio),
      "Risk-adjusted return after the available strategy checks.",
      sharpeTone(sharpeRatio),
      70,
    ),
    evidence(
      "profit-factor",
      "Profit factor",
      decimalValue(profitFactor),
      "Gross profit divided by gross loss.",
      profitFactorTone(profitFactor),
      80,
    ),
    evidence(
      "max-drawdown",
      "Max drawdown",
      pctValue(maxDrawdown),
      "Largest peak-to-trough loss in the available history.",
      inverseScoreTone(maxDrawdown, 25),
      90,
    ),
    evidence(
      "data-reliability",
      "Data reliability",
      pctValue(dataReliability),
      "Freshness and coverage of quotes, signals, and validation data.",
      scoreTone(dataReliability),
      100,
    ),
    evidence(
      "calibration-trustworthiness",
      "Calibration trustworthiness",
      pctValue(calibrationTrustworthiness),
      "How trustworthy calibrated confidence is relative to raw confidence.",
      scoreTone(calibrationTrustworthiness),
      110,
    ),
    evidence(
      "walk-forward-stability",
      "Walk-forward stability",
      pctValue(walkForward),
      "Stability across independent test periods.",
      scoreTone(walkForward),
      120,
    ),
    evidence(
      "readiness-score",
      "Readiness score",
      pctValue(readinessScore),
      "Overall production or live-test readiness.",
      scoreTone(readinessScore),
      130,
    ),
    evidence(
      "wisdom-score",
      "Wisdom score",
      pctValue(wisdomScore),
      "Decision quality after opportunity cost, counterfactuals, Agency value, and portfolio reasoning.",
      scoreTone(wisdomScore),
      140,
    ),
    evidence(
      "decision-quality",
      "Decision quality",
      pctValue(decisionQuality),
      "How the proposed action compares with alternatives and historical outcomes.",
      scoreTone(decisionQuality),
      150,
    ),
    evidence(
      "restriction-value",
      "Restriction value",
      pctValue(restrictionValue),
      "Whether restrictions saved more downside than they sacrificed upside.",
      scoreTone(restrictionValue),
      160,
    ),
    evidence(
      "opportunity-cost",
      "Opportunity cost",
      decimalValue(opportunityCost),
      "Utility sacrificed by not taking the best available option.",
      inverseScoreTone(opportunityCost, 25),
      170,
    ),
    evidence(
      "discovery-intelligence-score",
      "Discovery intelligence",
      pctValue(discoveryIntelligenceScore),
      "Whether discoveries, governance, economics, institutional knowledge, and meta-learning create durable value.",
      scoreTone(discoveryIntelligenceScore),
      180,
    ),
  ];
}
...
export function buildDecisionPipeline(
  ...
  const discoveryScore = firstFinite(
    input.discovery?.confidence,
    input.discoveryDensity?.confidence,
    input.discoveryPipeline?.averageScore,
  );
  const recognitionScore = firstFinite(
    input.recognition?.recognitionScore,
    input.recognition?.recurrenceConfidence,
  );
  const beliefScore = firstFinite(
    input.belief?.confidence,
    input.belief?.trustworthiness,
  );
  const judgementScore = firstFinite(
    input.judgement?.adjustedConfidence,
    input.judgement?.reliability,
  );
  const agencyScore = agencyTrustScore(input);
  const wisdomScore = firstFinite(
    input.wisdom?.wisdomScore,
    input.wisdom?.decisionQuality,
  );
  const resolveScore = firstFinite(input.resolve?.resolveScore);
  const survivalScore = firstFinite(
    input.survivalMemory?.survivalConfidence,
    input.survivalMemory?.score,
    input.survivalMemory?.survivalScore,
  );
  const discoveryIntelligenceScore = firstFinite(
    input.discoveryIntelligence?.score,
  );
  ...
  return [
    {
      ...
      status: readable(
        input.discovery?.status ??
          input.discovery?.lifecycle?.status ??
          "pending",
      ),
      score: discoveryScore,
      confidenceLabel: pctValue(discoveryScore),
      reason: firstMeaningful(
        input.discovery?.recommendedNextStep,
        input.discoveryDensity?.explanation,
        input.discovery?.explanation,
        "Discovery evidence is pending.",
      ),
      outcome: outcomeFromScoreStatus(discoveryScore, input.discovery?.status),
      nextRequiredImprovement: firstMeaningful(
        input.discovery?.recommendedNextStep,
        firstUnlockMatching(input, /discovery|candidate|density/i),
        "Mature candidate density and confidence.",
      ),
    },
    {
      stage: "Recognition",
      ...
      reason: firstMeaningful(
        ...
        "Recognition is waiting for comparable state evidence.",
      ),
      outcome: recognitionOutcome(input.recognition),
      nextRequiredImprovement: firstMeaningful(
        firstString(input.recognition?.missingEvidence),
        firstUnlockMatching(input, /recognition|recurrence|sample/i),
        "Increase comparable state and outcome evidence.",
      ),
    },
    {
      stage: "Belief",
      ...
      reason: firstMeaningful(
        input.belief?.reason,
        "Belief evidence is pending.",
      ),
      outcome: beliefOutcome(input.belief),
      nextRequiredImprovement: firstMeaningful(
        firstString(input.belief?.blockers),
        firstString(input.belief?.warnings),
        "Resolve contradictory evidence and improve evidence agreement.",
      ),
    },
    {
      stage: "Judgement",
      ...
      reason: firstMeaningful(
        firstString(input.judgement?.reasons),
        "Judgement is waiting for similar historical outcomes.",
      ),
      outcome: judgementOutcome(input.judgement),
      nextRequiredImprovement: firstMeaningful(
        firstString(input.judgement?.warnings),
        firstUnlockMatching(input, /judgement|similar|outcome/i),
        "Raise reliability and outcome stability.",
      ),
    },
    {
      stage: "Agency",
      ...
      reason: firstMeaningful(
        firstString(input.agency?.reasons),
        firstString(input.agencyDiagnostics?.state?.selfDiagnosis?.reasons),
        "Agency diagnostics are pending.",
      ),
      outcome: agencyOutcome(input),
      nextRequiredImprovement: firstMeaningful(
        firstUnlockMatching(input, /agency|human review|policy/i),
        "Clear agency review and policy requirements.",
      ),
    },
    {
      stage: "Resolve",
      ...
      reason: firstMeaningful(
        ...
        "Resolve will appear after upstream evidence is evaluated.",
      ),
      outcome: resolveOutcome(input.resolve),
      nextRequiredImprovement: firstMeaningful(
        firstString(input.resolve?.unlockConditions),
        firstString(input.resolve?.missingEvidence),
        "Clear the primary resolve gate.",
      ),
    },
    {
      stage: "Wisdom",
      ...
      reason: firstMeaningful(
        ...
        "Wisdom is waiting for decision outcome memory and counterfactual evidence.",
      ),
      outcome: outcomeFromScoreStatus(
        wisdomScore,
        input.wisdom ? "evaluated" : "pending",
      ),
      nextRequiredImprovement: firstMeaningful(
        input.wisdom?.counterfactuals?.missedUpside &&
          ...
          : "",
        input.wisdom?.learningConfidence != null &&
          input.wisdom.learningConfidence < 60
    ...
    },
    {
      ...
      reason: firstMeaningful(
        firstString(input.survivalMemory?.mainWarnings),
        firstString(input.survivalMemory?.reasons),
        "Survival memory is waiting for comparable outcome evidence.",
      ),
      outcome: outcomeFromScoreStatus(
        survivalScore,
        input.survivalMemory?.status,
      ...
      nextRequiredImprovement: firstMeaningful(
        firstString(input.survivalMemory?.unlockConditions),
        "Clear active survival restrictions and preserve recovery evidence.",
      ),
    },
    {
      ...
      reason: firstMeaningful(
        firstString(
          input.discoveryIntelligence?.recommendations?.map(
            (item) => item.message,
          ),
        ),
        "Discovery Intelligence is waiting for lifecycle, economics, governance, institutional, and outcome traces.",
      ),
      outcome: outcomeFromScoreStatus(
        discoveryIntelligenceScore,
        input.discoveryIntelligence ? "evaluated" : "pending",
      ),
      nextRequiredImprovement: firstMeaningful(
        firstString(
          input.discoveryIntelligence?.recommendations?.map(
        ...
        "Record outcomes for discoveries, decisions, restrictions, and institutional promotions.",
      ),
    },
  ];
}
...
export function extractUnlockInvalidationConditions(
  ...
  const unlockConditions = uniqueStrings([
    ...
    ...toStringArray(
      input.executive?.unlockConditions?.map(
        (condition: any) => condition.description ?? condition,
      ),
    ),
    ...
    ...(input.wisdom?.opportunityEconomics?.opportunityCost &&
    input.wisdom.opportunityEconomics.opportunityCost > 25
      ? [
  ...
  const invalidationConditions = uniqueStrings([
    ...
    ...toStringArray(
      input.executive?.invalidationConditions?.map(
        (condition: any) => condition.description ?? condition,
      ),
    ),
    ...toStringArray(input.trustGovernor?.contradictions),
    ...(input.wisdom?.decisionQuality != null &&
    input.wisdom.decisionQuality < 35
      ? [
  ...
  return {
    ...
    primaryUnlockCondition:
      unlockConditions[0] ??
      "Collect more evidence until the primary limiting gate clears.",
    primaryInvalidationCondition:
      invalidationConditions[0] ?? DEFAULT_INVALIDATION,
  };
}
...
export function buildDecisionChangeMap(
  ...
  const normalUnlock =
    canonicalReasons.find(
      (reason) =>
        reason.code === "survival_scar" ||
        reason.code === "recovery_incomplete",
    )?.unlockCondition ??
    canonicalReasons.find((reason) => reason.code === "trust_below_threshold")
      ?.unlockCondition ??
    primaryUnlock;

  return {
    increaseExposure: increaseExposure.length
      ? increaseExposure
      : [
          "Clear the active restriction reasons and preserve evidence stability.",
        ],
    reduceExposure,
    invalidateSignal: invalidateSignal.length
      ? invalidateSignal
...
export function buildTerminologyGroups(
  input: ExecutiveDashboardInput = {},
): TerminologyGroup[] {
  const trustScore = firstFinite(input.trustGovernor?.trustScore);
  const trustworthiness = firstFinite(
    input.calibration?.trustworthiness,
    input.belief?.trustworthiness,
  );
  ...
  const rawConfidence = firstFinite(
    input.calibration?.rawConfidence,
    input.judgement?.rawConfidence,
  );
  const calibratedConfidence = firstFinite(
    input.calibration?.calibratedConfidence,
    input.judgement?.adjustedConfidence,
  );
  const adjustedConfidence = firstFinite(input.judgement?.adjustedConfidence);
  const confidenceCap = firstFinite(
    input.trustGovernor?.confidenceCap,
    input.readiness?.maxConfidence,
  );
  const reliability = firstFinite(
    input.judgement?.reliability,
    input.strategyHistory?.modelReliability,
    input.readiness?.modelReliability,
  );
  const calibrationHealth = firstFinite(
    input.agency?.calibrationHealthPct != null
      ? input.agency.calibrationHealthPct * 100
      : null,
    input.calibration?.trustworthiness,
  );
  const survivalConfidence = firstFinite(
    input.survivalMemory?.survivalConfidence,
  );
  ...
  const candidateQuality = firstFinite(
    input.opportunity?.candidateQualityPct,
    input.discoveryDensity?.quality,
    input.discoveryPipeline?.averageScore,
  );
  const discoveryConfidence = firstFinite(
    input.discovery?.confidence,
  ...
  const maturityScore = firstFinite(
    input.discoveryIntelligence?.maturity.maturityScore,
  );
  const governanceScore = firstFinite(
    input.discoveryIntelligence?.governance.score,
  );
  const metaLearningScore = firstFinite(
    input.discoveryIntelligence?.metaLearning.score,
  );
  ...
  return [
    ...
    group(
      "Confidence",
      "How strongly the current signal is believed before and after calibration.",
      [
        metric("Raw confidence", pctValue(rawConfidence), "Calibration"),
        metric(
          "Calibrated confidence",
          pctValue(calibratedConfidence),
          "Calibration",
        ),
        metric(
          "Adjusted confidence",
          pctValue(adjustedConfidence),
          "Judgement",
        ),
    ...
    group("Reliability", "How stable the evidence and model history are.", [
      metric(
        "Data reliability",
        pctValue(dataReliabilityScore(input)),
        "Reliability",
      ),
      metric(
        "Model reliability",
        pctValue(reliability),
        "Judgement / Readiness",
      ),
      metric("Calibration health", pctValue(calibrationHealth), "Calibration"),
      metric(
        "Walk-forward stability",
        pctValue(walkForwardStability(input)),
        "Strategy History",
      ),
    ]),
    group(
      "Safety",
      "Whether prior damage, recovery, and risk controls allow exposure.",
      [
        metric(
          "Survival memory",
          readable(input.survivalMemory?.status ?? "pending"),
          "Survival Memory",
        ),
        metric(
          "Survival confidence",
          pctValue(survivalConfidence),
          "Survival Memory",
        ),
    ...
    group(
      "Opportunity",
      "Whether enough high-quality candidates exist to justify participation.",
      [
        metric(
          "Opportunity density",
          pctValue(opportunityDensity),
          "Discovery",
        ),
        metric("Candidate quality", pctValue(candidateQuality), "Discovery"),
        metric(
          "Discovery confidence",
          pctValue(discoveryConfidence),
          "Discovery",
        ),
      ],
    ),
    group(
      "Wisdom",
      "Whether the action is justified against alternatives, restrictions, and capital use.",
      [
        metric("Wisdom score", pctValue(wisdomScore), "Wisdom"),
        metric(
          "Restriction value",
          pctValue(restrictionValue),
          "Wisdom / Counterfactuals",
        ),
        metric(
          "Opportunity cost",
          decimalValue(opportunityCost),
          "Wisdom / Opportunity Economics",
        ),
      ],
    ),
    group(
      "Discovery Intelligence",
      "Whether discovery behavior creates value and becomes trusted reusable knowledge.",
      [
        metric(
          "Discovery Intelligence score",
          pctValue(discoveryIntelligenceScore),
        ...
        metric(
          "Maturity score",
          pctValue(maturityScore),
          "Discovery Intelligence",
        ),
        metric(
          "Governance score",
          pctValue(governanceScore),
          "Discovery Intelligence",
        ),
        metric(
          "Meta-learning score",
          pctValue(metaLearningScore),
          "Discovery Intelligence",
        ),
      ],
    ),
...
function buildExecutiveReasoning(
  ...
  const recoveryPhrase = canonicalReasons.some(
    (reason) =>
      reason.code === "recovery_incomplete" || reason.code === "survival_scar",
  )
...
function buildConfidenceLedger(
  ...
  const calibrated = firstFinite(
    input.calibration?.calibratedConfidence,
    input.judgement?.adjustedConfidence,
  );
  const survival = firstFinite(
    input.survivalMemory?.survivalConfidence,
    input.survivalMemory?.score,
    input.survivalMemory?.survivalScore,
  );
  const trust = firstFinite(
    input.trustGovernor?.trustScore,
    input.executive?.trust,
    input.calibration?.trustworthiness,
  );
  const discovery = firstFinite(
    input.discovery?.confidence,
    input.discoveryDensity?.confidence,
    input.discoveryIntelligence?.maturity.maturityScore,
  );
  ...
  const normalUnlock =
    canonicalReasons.find(
      (reason) =>
        reason.code === "survival_scar" ||
        reason.code === "recovery_incomplete",
    )?.unlockCondition ?? extracted.primaryUnlockCondition;
  const limitedUnlock =
    canonicalReasons.find((reason) => reason.code === "trust_below_threshold")
      ?.unlockCondition ??
    canonicalReasons.find((reason) => reason.code === "agency_unresolved")
      ?.unlockCondition ??
  ...
  const definitions: Array<Omit<ExposureStatePlan, "status">> = [
    {
      ...
      capPct: 0,
      entryRule:
        "Any hard veto, blocked permission, or missing minimum evidence.",
    ...
    },
    {
      ...
      entryRule:
        "Permission is limited, evidence is promising, and hard vetoes are absent.",
      exitRule:
        "Close a small outcome with controlled adverse excursion and usable liquidity.",
      unlockCondition:
        "Record a clean probe outcome with acceptable slippage, drawdown, and timing.",
      invalidationCondition:
        "Return to observe if probe adverse excursion breaches the survival boundary.",
    },
    {
      state: "micro",
      label: "Micro",
      ...
      entryRule:
        "Trust is provisional or survival memory is scarred, but action is allowed.",
      exitRule:
        "Promote only after reduced-size outcomes support survival and trust restoration.",
    ...
    },
    {
      ...
      entryRule:
        "Trust clears review, readiness is open, and discovery is at least confirmed.",
      exitRule:
        "Promote after survival, calibration, trust, and discovery gates remain stable.",
      unlockCondition: normalUnlock,
      invalidationCondition:
        "Reduce to micro if trust, opportunity density, or survival confidence regresses.",
    },
    {
      state: "normal",
      label: "Normal",
      ...
      entryRule:
        "Primary restrictions are clear and discovery is repeatable or trusted.",
      exitRule:
        "Downgrade immediately when a veto, contradiction, or adverse outcome appears.",
      unlockCondition:
        "Maintain trusted calibration, clear survival status, and repeatable discovery evidence.",
      invalidationCondition:
        "Invalidate normal sizing if any active veto reappears.",
    },
  ];
  const activeIndex = Math.max(
    0,
    definitions.findIndex((item) => item.state === activeState),
  );
...
function buildGovernanceConflicts(
  ...
  if (actionLeaning && (wisdomBest === "reject" || wisdomBest === "wait")) {
    conflicts.push({
      ...
      resolution:
        "Route the decision through governance review; larger sizing stays locked until the opportunity economics agree with the executive posture.",
    });
  }
  ...
  if (
    ...
    extracted.unlockConditions.some((condition) =>
      /survival confidence.*70|70.*survival confidence/i.test(condition),
    )
  ) {
    conflicts.push({
      ...
      resolution:
        "Treat the status and reduced-size outcome record as the authority; the scalar score alone cannot restore normal sizing.",
    });
  }
  ...
  if (
    ...
    rawConfidence - calibratedConfidence >= 15
  ) {
    conflicts.push({
      ...
      resolution:
        "Use calibrated confidence for trust, permission, and sizing until outcome stability closes the gap.",
    });
  }
...
function buildGovernanceVetoes(
  ...
  if (
    ...
    vetoes.push({
      ...
      severity: "hard",
      condition:
        firstString(decisionStates.permission.reasons) ??
  ...
  if (reasonCodes.has("data_reliability_low")) {
    vetoes.push({
      ...
      condition:
        canonicalReasons.find(
          (reason) => reason.code === "data_reliability_low",
        )?.label ?? "Data reliability low",
      effect:
        "Decision must wait or invalidate until evidence is synchronized.",
    });
  }
  ...
  if (reasonCodes.has("readiness_blocked")) {
    vetoes.push({
      ...
      severity: "hard",
      condition:
        canonicalReasons.find((reason) => reason.code === "readiness_blocked")
  ...
  for (const code of [
    ...
    vetoes.push({
      ...
      effect:
        code === "agency_unresolved"
          ? "Requires review before exposure increases."
          : "Caps exposure below normal sizing.",
    });
  }

  if (
    conflicts.some((conflict) => conflict.id === "executive-wisdom-conflict")
  ) {
    vetoes.push({
      source: "Wisdom",
      severity: "review",
  ...
  if (input.trustGovernor?.allowsNewExposure === false) {
    vetoes.push({
      ...
      condition:
        input.trustGovernor.primaryBlocker ??
        "Trust Governor blocks new exposure.",
...
function buildGovernanceDecisionCommand(
  ...
  const reason =
    primaryConflict?.resolution ??
    hardVeto?.effect ??
    mainReason?.explanation ??
    (action === "act"
      ? "Governance allows participation inside the active exposure state."
      : "Governance is waiting for clearer evidence.");
  ...
  return {
    ...
    reason,
    operatorInstruction: operatorInstructionFor(
      action,
      activeExposure?.state ?? "observe",
    ),
...
function buildRestrictionAccountability(
  ...
  const timeToRecovery = firstFinite(
    input.recovery?.audit?.timeToRecovery,
    input.survivalMemory?.recoveryBurden,
  );
  ...
  return canonicalReasons.slice(0, 6).map((reason) => {
    const audit = audits.find((item) => {
      ...
      return (
        haystack.includes(normalized(reason.label)) ||
        haystack.includes(reason.code.replace(/_/g, " "))
      );
    });
    ...
    const status = restrictionAccountabilityStatus(
      audit,
      avoidedLoss,
      missedUpside,
    );
    ...
    return {
      ...
      falseBlockRate,
      timeToRecovery,
      interpretation: restrictionAccountabilityInterpretation(
        status,
        avoidedLoss,
        missedUpside,
      ),
...
function buildAccountabilityLoop(
  ...
  const hasRestrictionAudit = restrictionBets.some(
    (bet) => bet.status === "helpful" || bet.status === "harmful",
  );
  const hasOutcomeMemory = Boolean(
    input.wisdom?.counterfactuals?.actualOutcome ||
      input.counterfactual?.scenarios?.length,
  );
  ...
  const hasInstitutionalAsset =
    firstFinite(
      input.discoveryIntelligence?.institutionalization.institutionalCount,
    ) != null &&
    Number(
      input.discoveryIntelligence?.institutionalization.institutionalCount,
    ) > 0;
  ...
  return [
    {
      ...
      evidenceRequired:
        "final decision, participation mode, exposure state, and active vetoes",
      nextAction: hasDecision
        ? "Preserve the decision snapshot for outcome review."
        : "Record the decision before evaluation continues.",
    },
    {
      id: "restriction-bet",
      label: "Restriction bet",
      status: hasRestrictionAudit
        ? "complete"
        : restrictionBets.length
          ? "pending"
          : "blocked",
      evidenceRequired:
        "avoided loss, missed upside, false block rate, and time to recovery",
      nextAction: hasRestrictionAudit
        ? "Compare restriction value against caution cost."
        : "Attach measurable economics to each active restriction.",
    },
    {
      id: "outcome-capture",
      ...
      evidenceRequired:
        "realized outcome, best alternative, worst alternative, and adverse excursion",
      nextAction: hasOutcomeMemory
        ? "Feed the result back into Wisdom and Discovery Intelligence."
        : "Capture the realized outcome when the review window closes.",
    },
    {
      id: "policy-adjustment",
      label: "Policy adjustment",
      status: needsPolicyReview
        ? "review"
        : hasRestrictionAudit
          ? "complete"
          : "pending",
      evidenceRequired:
        "restriction value versus caution cost and repeated conflict patterns",
      nextAction: needsPolicyReview
        ? "Review thresholds before the next comparable signal."
        : "Leave thresholds unchanged until restriction economics disagree.",
    },
    {
      ...
      evidenceRequired:
        "trusted discovery, policy, standard, or reusable governance rule",
      nextAction: hasInstitutionalAsset
        ? "Reuse the institutional rule in future comparable states."
        : "Promote repeatable discoveries into durable policy only after outcome proof.",
    },
  ];
}

function buildDiscoveryInstitutionalization(
  input: ExecutiveDashboardInput,
  ...
  const detectedStage = [...stages]
    .reverse()
    .find((stage) => stageCounts[stage] > 0);
  const currentStage =
    detectedStage ?? (normalized(input.discovery?.status) || "emerging");
  const currentIndex = Math.max(
    0,
    stages.indexOf(currentStage as keyof typeof stageCounts),
  );
  const nextStage = stages[Math.min(stages.length - 1, currentIndex + 1)];
  const allowedInfluence = institutionalInfluenceFor(
    currentStage,
    input.discoveryIntelligence?.score,
  );
  const recommendationMessages = toStringArray(
    input.discoveryIntelligence?.recommendations?.map((item) => item.message),
  );
...
function confidenceStatusFor(
  ...
  if (kind === "survival") {
    if (survivalStatus === "near ruin" || survivalStatus === "near_ruin")
      return "blocked";
...
function confidenceInterpretation(
  ...
  if (kind === "raw")
    return "Model conviction before outcome correction; it cannot directly authorize sizing.";
  if (kind === "calibrated")
    return "Sizing should trust this value before raw confidence when the two disagree.";
  if (kind === "trust")
    return "Governance trust controls whether confidence may become permission.";
  if (kind === "discovery") {
    const maturity = firstFinite(
      input.discoveryIntelligence?.maturity.maturityScore,
      input.discovery?.maturity,
    );
    if (maturity != null && maturity < 60)
      return "Discovery can guide attention and probes, but not normal sizing yet.";
...
function exposureStateFor(
  ...
  if (
    ["paper", "probe"].includes(mode) ||
    (maxExposure != null && maxExposure <= 0.3)
  )
    return "probe";
  if (
    mode === "micro" ||
    capacityMode === "micro" ||
    (maxExposure != null && maxExposure <= 0.75)
  )
    return "micro";
  if (
    [
...
function exposureCapFor(state: ExposureStateName, maxExposure: number | null) {
  ...
  if (state === "probe")
    return roundExposure(Math.min(nonZeroCap(cap, 0.25), 0.25));
  if (state === "micro")
    return roundExposure(Math.min(nonZeroCap(cap, 0.5), 0.5));
  if (state === "limited")
    return roundExposure(Math.min(nonZeroCap(cap, 1), 1));
...
function commandActionFor(
  ...
  if (hardVeto)
    return hardVeto.source === "Data Reliability" ? "invalidate" : "wait";
  ...
  if (
    /act|buy|commit|escalate/.test(finalDecision) &&
    decisionStates.permission.allowed &&
...
function commandToneFor(
  ...
  if (
    vetoes.some((veto) => veto.severity !== "hard") ||
...
function operatorInstructionFor(
  action: DecisionCommandAction,
  exposureState: ExposureStateName,
) {
  if (action === "act")
    return `Participation is allowed only inside ${readable(exposureState)} exposure.`;
  if (action === "wait")
    return "Do not add exposure until the active veto clears.";
  if (action === "reduce")
    return "Reduce or exit exposure according to the active risk gate.";
  if (action === "invalidate")
    return "Treat the signal as invalid until new evidence rebuilds the case.";
  return "Resolve the named conflict before increasing exposure.";
}

function nextAuditForCommand(
  action: DecisionCommandAction,
  mainReason?: CanonicalReason,
) {
  if (action === "act")
    return "Record realized return, max adverse excursion, slippage, and whether the limiter helped.";
  if (action === "wait")
    return "Record whether waiting avoided loss or created missed upside.";
  if (action === "reduce")
    return "Record avoided drawdown and the recovery time after reduction.";
  if (action === "invalidate")
    return "Record the failed condition and prevent automatic restoration without new evidence.";
  return mainReason
...
function restrictionAccountabilityInterpretation(
  ...
  if (status === "pending")
    return "Restriction economics are not yet measurable.";
  if (status === "needs_audit")
    return "Restriction value is flat; collect another comparable outcome.";
  if (status === "helpful")
    return `Avoided loss ${valueLabel(avoidedLoss)} exceeds missed upside ${valueLabel(missedUpside)}.`;
...
function institutionalInfluenceFor(stage: string, score: unknown) {
  ...
  if (normalizedStage === "institutional" || normalizedStage === "trusted")
    return "May affect sizing rules and reusable policy.";
  if (
    normalizedStage === "repeatable" ||
    normalizedStage === "confirmed" ||
    (intelligenceScore != null && intelligenceScore >= 60)
  ) {
...
function promotionRequirementFor(stage: string) {
  const normalizedStage = normalized(stage);
  if (normalizedStage === "institutional")
    return "Maintain policy performance through recurring audits.";
  if (normalizedStage === "trusted")
    return "Convert trusted discovery into a reusable policy or standard.";
  if (normalizedStage === "repeatable")
    return "Promote after repeated outcomes show stable value after costs.";
  if (normalizedStage === "confirmed")
    return "Observe recurrence across independent states before trusting sizing.";
  if (normalizedStage === "observed")
    return "Confirm with outcome memory and counterfactual review.";
...
function collectNarrativeText(input: ExecutiveDashboardInput) {
  return uniqueStrings([
    ...
    input.trustGovernor?.primaryBlocker,
    ...toStringArray(input.trustGovernor?.blockers).map(
      (blocker: any) => blocker?.reason ?? blocker?.label ?? blocker,
    ),
...
function reasonCodesFromText(value: string): CanonicalReasonCode[] {
  ...
  if (/survival|scar|near ruin|near-ruin|fragile/.test(text))
    codes.push("survival_scar");
  if (
    /trust.*below|below.*trust|trust governor|trust threshold|restoration threshold/.test(
      text,
    )
  )
    codes.push("trust_below_threshold");
  if (
    /reduced size|reduced-size|micro|limited exposure|small size|normal sizing/.test(
      text,
    )
  )
    codes.push("reduced_size");
  if (
    /recovery incomplete|recovering|recovery burden|restore|restoring/.test(
      text,
    )
  )
    codes.push("recovery_incomplete");
  if (/agency|human review|policy/.test(text)) codes.push("agency_unresolved");
  if (
    /opportunity density|candidate density|density.*low|transitional/.test(text)
  )
    codes.push("opportunity_density_low");
  if (/discovery confidence|discovery.*mature|novelty|maturity/.test(text))
    codes.push("discovery_immature");
  if (/calibration|calibrated|raw confidence|unstable outcomes/.test(text))
    codes.push("calibration_review");
  if (/readiness|execution gate|capacity|blocked/.test(text))
    codes.push("readiness_blocked");
  if (/overfit|robustness|too clean/.test(text)) codes.push("overfit_risk");
  if (/walk-forward|walk forward|segment|test period/.test(text))
    codes.push("walk_forward_instability");
  if (/data reliability|quote|signal coverage|freshness|stale/.test(text))
    codes.push("data_reliability_low");
  return codes;
}
...
function finalDecisionLabel(input: ExecutiveDashboardInput) {
  ...
  if (
    ...
    return "Watch";
  const sizingDecision = readable(
    input.sizing?.sizingDecision ?? input.sizing?.decision,
  );
...
function opportunityPhraseFor(input: ExecutiveDashboardInput) {
  if (
    input.recognition?.verdict === "recognized" &&
    firstFinite(input.recognition?.matchedPositiveOutcomes) != null
  ) {
...
function dataReliabilityScore(input: ExecutiveDashboardInput) {
  return firstFinite(
    ...
    input.agency?.dataReliabilityPct != null
      ? input.agency.dataReliabilityPct * 100
      : null,
...
function walkForwardStability(input: ExecutiveDashboardInput) {
  ...
  const walkForward =
    input.strategyHistory?.walkForward ??
    input.readiness?.walkForward ??
    input.strategyReadiness?.walkForward;
  const positive = firstFinite(
    walkForward?.positiveSegmentCount,
    walkForward?.positiveSegments,
  );
...
function agencyTrustScore(input: ExecutiveDashboardInput) {
  return firstFinite(
    ...
    input.agency?.trust,
    input.agencyDiagnostics?.summary?.averageTrust != null
      ? input.agencyDiagnostics.summary.averageTrust * 100
      : null,
    input.agencyDiagnostics?.state?.selfDiagnosis?.trust != null
      ? input.agencyDiagnostics.state.selfDiagnosis.trust * 100
      : null,
  );
}

function firstBlocker(trustGovernor: any) {
  return Array.isArray(trustGovernor?.blockers)
    ? trustGovernor.blockers[0]
    : null;
}
...
function firstInvalidationMatching(
  input: ExecutiveDashboardInput,
  pattern: RegExp,
) {
...
function outcomeFromScoreStatus(
  score: number | null,
  status: unknown,
...
function judgementOutcome(judgement: any): ExecutivePipelineOutcome {
  ...
  if (status === "review required" || status === "review_required")
    return "escalated";
  return "limited";
}

function agencyOutcome(
  input: ExecutiveDashboardInput,
): ExecutivePipelineOutcome {
  ...
  if (/human review|requires_human_review|review/.test(recommendation))
    return "escalated";
...
function group(
  concept: TerminologyGroup["concept"],
  description: string,
  metrics: TerminologyGroup["metrics"],
): TerminologyGroup {
...
function toStringArray(value: unknown): string[] {
    ...
    return value
      .flatMap((item) => {
        ...
        if (typeof item === "object") {
          ...
          return String(
            record.reason ?? record.label ?? record.title ?? record.id ?? "",
          );
        }
        return String(item);
      })
  ...
  if (typeof value === "object") {
    ...
    return [
      String(record.reason ?? record.label ?? record.title ?? record.id ?? ""),
    ].filter(Boolean);
  }
  return [];
}
