...
export class SignalJournal {
  ...
  snapshot(): ValidationState {
    const evaluated = this.outcomes
      .map((outcome) => ({
        outcome,
        signal: this.signals.get(outcome.signalId),
      }))
      .filter(
        (entry): entry is { outcome: SignalOutcome; signal: SignalRecord } =>
          Boolean(entry.signal),
      );
    const returns = evaluated.map(({ outcome, signal }) =>
      directionalReturn(signal, outcome),
    );
    ...
    for (const entry of evaluated) {
      ...
      bucket.total += 1;
      if (entry.outcome.realizedDirection === entry.signal.expectedDirection)
        bucket.wins += 1;
      byRegime.set(entry.signal.regime, bucket);
    }

    const regimeAccuracy = Object.fromEntries(
      Array.from(byRegime.entries()).map(([regime, bucket]) => [
        regime,
        clamp((bucket.wins / Math.max(1, bucket.total)) * 100),
      ]),
    ) as Partial<Record<RegimeName, number>>;
    const calibrationErrors = evaluated.map(({ outcome, signal }) =>
      Math.abs(Math.abs(outcome.realizedMagnitude) - signal.expectedMagnitude),
    );
    ...
    return {
      ...
      confidenceRealism: evaluated.length
        ? clamp((wins / evaluated.length) * 100)
        : 72,
