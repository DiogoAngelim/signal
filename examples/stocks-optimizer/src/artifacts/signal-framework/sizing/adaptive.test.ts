...
test("gather-evidence needs reduce sizing through soft constraints", () => {
  ...
  assert.equal(
    result.constraints.some(
      (constraint) =>
        constraint.id === "need:gather-evidence" && !constraint.passed,
    ),
    true,
  );
});
...
test("custom ladders are normalized and sizing modes follow selected steps", () => {
  const micro = sizeAdaptiveOpportunity(
    base({
      requestedCapacity: 1,
      availableCapacity: 1,
      maxCapacity: 1,
  ...
  const small = sizeAdaptiveOpportunity(
    base({ requestedCapacity: 10, availableCapacity: 10, maxCapacity: 10 }),
  );
  const normal = sizeAdaptiveOpportunity(
    base({ requestedCapacity: 25, availableCapacity: 25, maxCapacity: 25 }),
  );
  ...
  const large = sizeAdaptiveOpportunity(
    base({
      ...highConviction,
  ...
  const maxSafe = sizeAdaptiveOpportunity(
    base({
      ...highConviction,
  ...
  const none = sizeAdaptiveOpportunity(
    base({ requestedCapacity: 0, availableCapacity: 0, maxCapacity: 0 }),
  );
...
function expectSizingBlocked(
  result: ReturnType<typeof sizeAdaptiveOpportunity>,
  id: string,
) {
  assert.equal(result.decision, "blocked");
