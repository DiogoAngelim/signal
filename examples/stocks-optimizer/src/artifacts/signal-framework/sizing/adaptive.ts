...
export const ADAPTIVE_SIZING_LADDER = [
...
export function sizeAdaptiveOpportunity(
  input: AdaptiveSizingInput,
  ...
  const risk = Math.max(
    ratio(input.risk ?? 1 - riskControl),
    1 - riskControl,
    needRisk,
  );
  const blendedConfidence = mean([
    quality,
    confidence,
    alignment,
    trust,
  ...
  const utility = mean([
    ...
    participation,
    discovery,
    needParticipationBoost(input.needs ?? []),
  ]);
  const constraints = [
    ...(input.constraints ?? []),
    ...constraintsFromNeeds(input.needs ?? []),
  ];
  ...
  const normalizedSize = normalizeByCapacity(
    finalSize,
    input.maxCapacity ?? input.availableCapacity ?? input.requestedCapacity,
  );
  ...
  return ratio(
    Math.max(
      ...
      ...needs
        .filter(
          (need) =>
            need.category === "reduce-exposure" || need.category === "wait",
        )
...
function needParticipationBoost(needs: DetectedNeed[]) {
  const increase = needs.find(
    (need) => need.category === "increase-participation",
  );
  return increase ? ratio(increase.confidence) : 0.5;
}

function selectLadderStep(ladder: number[], allowedCapacity: number) {
  const selected = ladder.reduce(
    (stepSelected, step) => (step <= allowedCapacity ? step : stepSelected),
    0,
  );
...
function normalizeLadder(ladder: readonly number[]) {
  return Array.from(
    new Set(
      ladder.map((step) => Math.max(0, numeric(step))).filter(Number.isFinite),
    ),
  ).sort((a, b) => a - b);
}
