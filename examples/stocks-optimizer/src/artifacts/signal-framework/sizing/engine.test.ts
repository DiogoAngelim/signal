...
import { type SizingInput, sizeDecision } from "./engine";
...
test("soft and non-blocking hard constraints reduce size without blocking", () => {
  ...
  const constrained = sizeDecision(
    base({
  ...
  assert.ok(
    constrained.reasons.includes(
      "soft-limit failed with high severity; sizing reduced.",
    ),
  );
  assert.ok(constrained.reasons.includes("Soft governor is weak."));
  assert.ok(
    constrained.reasons.includes(
      "hard-medium failed with medium severity; sizing reduced.",
    ),
  );
  assert.ok(
    constrained.reasons.includes(
      "fallback-severity failed with medium severity; sizing reduced.",
    ),
  );
});

test("confidence scaling increases size as confidence improves", () => {
  const low = sizeDecision(
    base({ confidence: 0.2, risk: 0.2, availableCapacity: 100 }),
  );
  const moderate = sizeDecision(
    base({ confidence: 0.5, risk: 0.2, availableCapacity: 100 }),
  );
  const strong = sizeDecision(
    base({ confidence: 0.8, risk: 0.2, availableCapacity: 100 }),
  );

  assert.ok(low.size < moderate.size);
  assert.ok(moderate.size < strong.size);
  assert.ok(
    low.reasons.includes("Confidence 20% is low; sizing is conservative."),
  );
  assert.ok(
    moderate.reasons.includes(
      "Confidence 50% is moderate; sizing remains limited.",
    ),
  );
...
test("risk scaling reduces size as risk rises", () => {
  const controlled = sizeDecision(
    base({ confidence: 0.7, risk: 0.2, availableCapacity: 100 }),
  );
  const moderate = sizeDecision(
    base({ confidence: 0.7, risk: 0.55, availableCapacity: 100 }),
  );
  const high = sizeDecision(
    base({ confidence: 0.7, risk: 0.85, availableCapacity: 100 }),
  );
  const maximum = sizeDecision(
    base({ confidence: 0.7, risk: 1, availableCapacity: 100 }),
  );
  ...
  assert.ok(
    moderate.reasons.includes("Risk 55% is moderate; sizing is controlled."),
  );
  assert.ok(high.reasons.includes("Risk 85% is high; sizing is reduced."));
  assert.ok(
    maximum.reasons.includes("Risk is at maximum; sizing starts at zero."),
  );
});

test("utility and fully passed constraints can adjust size upward or downward", () => {
  const lowUtility = sizeDecision(
    base({
      confidence: 0.8,
      risk: 0.2,
      utility: 0.2,
  ...
  const highUtility = sizeDecision(
    base({
      confidence: 0.8,
      risk: 0.2,
      utility: 0.8,
      availableCapacity: 100,
      constraints: [
        { id: "all-clear", type: "soft", passed: true, severity: "low" },
      ],
    }),
  );
...
test("capacity caps respect requested, maximum, and available capacity", () => {
  const requestedAndMax = sizeDecision(
    base({
      confidence: 1,
      risk: 0,
  ...
  assert.deepEqual(requestedAndMax.audit.cappedBy, [
    "requestedCapacity",
    "maxCapacity",
  ]);
  assert.equal(available.size, 30);
  assert.deepEqual(available.audit.cappedBy, [
    "maxCapacity",
    "availableCapacity",
  ]);
...
test("minimum capacity is applied only when caps can satisfy it", () => {
  ...
  const raisedWithoutCaps = sizeDecision(
    base({
      confidence: 0.1,
      risk: 0,
      minCapacity: 0.2,
  ...
  assert.ok(
    notRaised.reasons.includes(
      "Minimum capacity 15 could not be met within caps.",
    ),
  );
...
test("normalized size stays in bounds and classifies modes deterministically", () => {
  ...
  assert.ok(
    clamped.reasons.includes("Confidence was outside 0-100%; clamped to 100%."),
  );
  assert.ok(
    clamped.reasons.includes("Risk was outside 0-100%; clamped to 0%."),
  );
});
...
test("invalid and missing values use conservative deterministic defaults", () => {
  ...
  assert.ok(
    result.reasons.includes("Confidence was missing or invalid; using 0%."),
  );
  assert.ok(
    result.reasons.includes("Risk was missing or invalid; using 100%."),
  );
  assert.ok(
    result.reasons.includes("Requested capacity was invalid and ignored."),
  );
  assert.ok(
    result.reasons.includes("Maximum capacity was below zero; using 0."),
  );
