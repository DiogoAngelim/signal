...
export type PurposeInput = {
  ...
  wisdom?:
    | Partial<DecisionQualityResult | WisdomSummary>
    | PurposeModuleContext
    | null;
...
export function translateAmbition(ambition: number): {
  ...
  const exponential =
    ((Math.exp(declaredAmbition / 22) - 1) / (Math.exp(100 / 22) - 1)) * 100;
  const ambitionIntensity = roundScore(
    declaredAmbition * 0.48 + exponential * 0.52,
  );
  ...
  return {
    ...
    purposeProfile: {
      ...
      preservationPreference: roundScore(
        88 - ambitionIntensity * 0.48 + lowAmbitionPull * 0.08,
      ),
      certaintyPreference: roundScore(90 - ambitionIntensity * 0.58),
      opportunityPreference: roundScore(14 + ambitionIntensity * 0.86),
      patiencePreference: roundScore(
        86 - Math.abs(declaredAmbition - 42) * 0.34 + lowAmbitionPull * 0.05,
      ),
      recoveryPreference: roundScore(
        55 + ambitionIntensity * 0.22 + lowAmbitionPull * 0.18,
      ),
      ...
      survivalPriority: roundScore(
        Math.max(
          58,
          84 - ambitionIntensity * 0.22 + extremeAmbitionPull * 0.45,
        ),
      ),
    },
  };
...
export function evaluatePurpose(input: PurposeInput): PurposeResult {
  ...
  const clarityScore = firstScore(
    input.currentPath?.clarity,
    frictionScore,
    70,
  );
  const usefulnessScore = firstScore(
    input.currentPath?.usefulness,
    moduleScores.wisdom,
    input.currentPath?.progress,
    62,
  );
  const baseSurvivabilityScore = survivalScoreFor(
    input,
    purposeProfile,
    pruning,
  );
  ...
  const behaviorFitScore = roundScore(
    mean([
  ...
  const sustainableProgressScore = roundScore(
    mean([
  ...
  const goalProgressScore = roundScore(
    firstScore(
      input.currentPath?.progress,
      averageDefined(
        expectations.map((record) => finiteMaybe(record.progress)),
      ),
  ...
  const desiredFutureAlignment = roundScore(
    firstScore(
      ...
      mean([
        goalProgressScore,
        purposeProfile.growthPreference,
        100 - purposeProfile.certaintyPreference * 0.2,
      ]),
    ),
  );
  ...
  const alignmentTrustScore = roundScore(
    mean([
      behaviorFitScore,
      expectationCalibrationScore,
      clarityScore,
  ...
  const advocacyScore = roundScore(
    mean([
      satisfactionScore,
      alignmentTrustScore,
      clarityScore,
  ...
  const purposeScore = roundScore(
    Math.min(rawPurposeScore, 40 + survivabilityScore * 0.6),
  );
  const evidenceQuality = firstScore(
    input.evidenceQuality,
    input.currentPath?.evidenceQuality,
    moduleScores.evidence,
    58,
  );
  const sampleConfidence = roundScore(
    Math.min(
      100,
      (behavior.length + expectations.length + frictionRecords.length) * 9,
    ),
  );
...
function learnBehavioralIdentity(input: {
  ...
  const patienceScore = observedScore(
    behavior,
    "patience",
    purposeProfile.patiencePreference,
  );
  const disciplineScore = observedScore(behavior, "discipline", 62);
  const consistencyScore = observedScore(behavior, "consistency", 62);
  const recoveryScore = observedScore(
    behavior,
    "recovery",
    purposeProfile.recoveryPreference,
  );
  const convictionScore = observedScore(
    behavior,
    "conviction",
    purposeProfile.opportunityPreference,
  );
  const adaptationScore = observedScore(behavior, "adaptation", 60);
  const stressToleranceScore = observedScore(
    behavior,
    "stressTolerance",
    purposeProfile.volatilityAcceptance,
  );
  const confidenceCalibrationScore = observedScore(
    behavior,
  ...
  const abandonmentRate = ratio(
    behavior,
    (record) => record.abandoned === true,
  );
  const sustainedRate = ratio(
    behavior,
    (record) => record.sustainedProgress === true || record.completed === true,
  );
  const regret = averageDefined(
    behavior.map((record) => finiteMaybe(record.regret)),
  );
  const observedAmbition = ambitionSignals.length
    ? mean(ambitionSignals)
    : behaviorCore * 0.72 + ambition * 0.28;
  const behavioralAmbition = evidenceCount
    ...
    : ambition;
  const identityConflictScore = roundScore(
    Math.abs(ambition - behavioralAmbition),
  );
...
function survivalScoreFor(
  input: PurposeInput,
  profile: PurposeProfile,
  pruning: NormalizedPruning,
) {
  const direct = firstScoreMaybe(
    input.currentPath?.survivability,
    input.survivalScore,
    input.decision?.survivability,
  );
  const recovery = moduleScore(input.recovery, [
    "recoveryScore",
    "score",
    "trustedCapacity",
  ]);
  const governance = moduleScore(input.governance, [
    "score",
    "trust",
    "governanceScore",
  ]);
  const pruningSurvival = pruning.survivalContribution || undefined;
  const blended = roundScore(
    mean([
...
function normalizeMeaning(
  ...
  const gravityScore = clamp(
    numeric(
      record.gravityScore,
      numeric((purposeInputs as Record<string, unknown>).gravityScore, 0),
    ),
    -10,
    10,
  );
  const needConfidence = clamp(
    numeric(
      record.needConfidence,
      numeric((purposeInputs as Record<string, unknown>).needConfidence, 0.5),
    ),
    0,
    1,
  );
  ...
  const actionPermission = String(
    record.purposeInputs?.actionPermission ??
      (gravityScore <= -9
        ? "block"
        : gravityScore <= -7
          ? "review"
  ...
  const safetyPriority = clamp(
    numeric(
      record.purposeInputs?.safetyPriority,
  ...
  return {
    ...
    needConfidence,
    positiveGoal: String(
      record.positiveGoal ?? record.purposeInputs?.positiveGoal ?? "",
    ),
    transformedGoal: String(
      record.transformedGoal ?? record.purposeInputs?.transformedGoal ?? "",
    ),
    literalDesireUnsafe,
    actionPermission,
    safetyPriority,
...
function normalizePruning(input: PurposeInput["pruning"]): NormalizedPruning {
  ...
  const recommendedAction = Array.isArray(input)
    ? mostSeverePruningAction(
        candidates.map((candidate) =>
          String(candidate.recommendedAction ?? "keep"),
        ),
      )
    : String(
  ...
  const pruningScore = Array.isArray(input)
    ? (averageDefined(
        candidates.map((candidate) => finiteMaybe(candidate.pruningScore)),
      ) ?? 0)
    : firstScore(
        input?.pruningScore,
        averageDefined(
  ...
  const evidenceConfidence = Array.isArray(input)
    ? (averageDefined(
        candidates.map((candidate) =>
          finiteMaybe(candidate.evidenceConfidence),
        ),
      ) ?? 65)
    : firstScore(
  ...
  const survivalContribution = Array.isArray(input)
    ? (averageDefined(
        candidates.map((candidate) =>
          finiteMaybe(candidate.survivalContribution),
        ),
      ) ?? 45)
    : firstScore(
  ...
  return {
    recommendedAction,
    falseConfidenceRisk: roundScore(
      Math.max(
        actionRisk,
        pruningScore * 0.42 + Math.max(0, 45 - evidenceConfidence) * 0.65,
      ),
    ),
    evidenceConfidence,
...
function collectModuleScores(input: PurposeInput) {
  const wisdom = moduleScore(input.wisdom, [
    "wisdomScore",
    "score",
    "decisionQuality",
    "learningConfidence",
  ]);
  const selfModel = moduleScore(input.selfModel, [
    "score",
    "trust",
    "confidence",
    "selfModelScore",
  ...
  const outcome = moduleScore(input.outcome, [
    "score",
    "progress",
    "value",
    "outcomeScore",
  ]);
  const counterfactual = moduleScore(input.counterfactual, [
    "score",
    "decisionQuality",
  ...
  const causal = moduleScore(input.causal, [
    "score",
    "confidence",
    "causalScore",
  ]);
...
function decisionPriority(input: {
  ...
  const returnPressure = firstScore(
    decision?.expectedReturn,
    decision?.expectedValue,
    decision?.priority,
    50,
  );
  const explicitAlignment = firstScoreMaybe(
    decision?.alignment,
    input.input.currentPath?.alignment,
  );
  const effectiveAlignment =
    explicitAlignment == null
      ? input.alignmentScore
      : Math.min(input.alignmentScore, explicitAlignment);
  const explicitSurvival = firstScoreMaybe(
    decision?.survivability,
    input.input.currentPath?.survivability,
    input.input.survivalScore,
  );
  const effectiveSurvival =
    explicitSurvival == null
  ...
  if (input.meaning?.actionPermission === "review") {
    return {
      ...
      priorityAdjustment: roundScore(
        -26 + input.meaning.gravityScore * 0.8,
        -100,
        100,
      ),
    };
  ...
  if (input.meaning?.actionPermission === "reduce") {
    return {
      ...
      priorityAdjustment: roundScore(
        -18 + input.meaning.gravityScore * 0.5,
        -100,
        100,
      ),
    };
  ...
  if (input.expectationCalibrationScore < 48) {
    return {
      ...
      priorityAdjustment: roundScore(
        (input.expectationCalibrationScore - 60) * 0.5,
        -100,
        100,
      ),
    };
  ...
  if (returnPressure >= 72 && effectiveAlignment < 55) {
    return {
      recommendedAction: "reduce-priority",
      priorityAdjustment: roundScore(
        (effectiveAlignment - returnPressure) * 0.45,
        -100,
        100,
      ),
    };
  ...
  if (
    input.alignmentScore >= 72 &&
    input.survivabilityScore >= 68 &&
    input.purposeConfidence >= 58
  ) {
    return {
      recommendedAction: "increase-priority",
      priorityAdjustment: roundScore(
        (input.alignmentScore + input.survivabilityScore - 120) * 0.3,
        -100,
        100,
      ),
    };
...
function buildPurposeStatement(input: {
  ...
  const effectiveAmbition = Math.min(
    input.ambition,
    Math.max(input.behavioralAmbition, input.ambition * 0.72),
  );
  const sacrifice =
    effectiveAmbition >= 82
...
function warningsFor(input: {
  ...
  if (input.identityConflict >= 24)
    warnings.push(
      "Declared ambition and observed behavior are materially different.",
    );
  if (input.survivabilityScore < 55)
    warnings.push(
      "Survival protection is low; Purpose must reduce confidence before increasing priority.",
    );
  if (input.expectationCalibrationScore < 55)
    warnings.push("Expectation mismatch is likely to reduce satisfaction.");
  if (input.regretScore < 55)
    warnings.push(
      "Regret pressure is high enough to threaten long-term trust.",
    );
  if (input.frictionScore < 55)
    warnings.push(
      "Friction is high; simplify the path before asking for more participation.",
    );
  if (input.purposeConfidence < 50)
    warnings.push(
      "Purpose confidence is limited by weak evidence or identity conflict.",
    );
  if (input.pruning.falseConfidenceRisk >= 25)
    warnings.push("Pruning found evidence that could create false confidence.");
  if (input.meaning?.literalDesireUnsafe)
    warnings.push(
      "Meaning transformed an unsafe literal desire before Purpose alignment.",
    );
  if (input.meaning && input.meaning.needConfidence < 0.45)
    warnings.push(
      "Meaning confidence is low; Purpose should stay in degraded mode.",
    );
  for (const warning of input.meaning?.warnings ?? []) warnings.push(warning);
  for (const constraint of input.meaning?.safetyConstraints ?? [])
    warnings.push(`Meaning safety constraint: ${constraint}`);
  for (const warning of safeArray(warningsFrom(input.input.wisdom)))
    warnings.push(String(warning));
  for (const warning of safeArray(warningsFrom(input.input.selfModel)))
    warnings.push(String(warning));
  for (const warning of safeArray(warningsFrom(input.input.governance)))
    warnings.push(String(warning));
...
function buildTrace(input: {
  ...
  const traceEntries = [
    trace(
      ...
      input.alignmentScore,
      0.2,
      "How well the path advances the desired future and fits observed behavior.",
    ),
    trace(
      "satisfaction",
      "Satisfaction",
      ...
      0.17,
      "Probability the user feels helped toward the future they want.",
    ),
    trace(
      "retention",
      "Retention",
      input.retentionScore,
      0.14,
      "Probability the user can keep engaging over time.",
    ),
    trace(
      "advocacy",
      "Advocacy",
      input.advocacyScore,
      0.1,
      "Probability the user recommends the system as clear and useful.",
    ),
    trace(
      "progress",
      "Goal progress",
      input.goalProgressScore,
      0.15,
      "Distance closed toward the desired future.",
    ),
    trace(
      "trust",
      "Alignment trust",
      input.alignmentTrustScore,
      0.14,
      "Probability the user believes the system understands them.",
    ),
    trace(
      "sustainable-progress",
      "Sustainable progress",
      input.sustainableProgressScore,
      0.1,
      "Progress adjusted by survivability, behavior fit, regret, friction, and expectations.",
    ),
    trace(
      "survival",
      "Survivability",
      input.survivabilityScore,
      1,
      "Purpose can never override survival.",
    ),
    trace(
      "expectations",
      "Expectation calibration",
      input.expectationCalibrationScore,
      1,
      "Reality compared with expected experience and outcomes.",
    ),
    trace(
      "regret",
      "Regret control",
      input.regretScore,
      1,
      "Disappointment, surprise, regret, and confidence shocks.",
    ),
    trace(
      "friction",
      "Friction control",
      input.frictionScore,
      1,
      "Complexity, mental effort, attention, and interaction burden.",
    ),
    trace(
      "confidence",
      "Purpose confidence",
      input.purposeConfidence,
      1,
      "Evidence strength after identity conflict and pruning risk.",
    ),
    trace(
      "identity-conflict",
      "Identity conflict",
      ...
      1,
      "Distance between declared ambition and observed behavior.",
    ),
  ];
  if (input.meaning) {
    traceEntries.push(
      trace(
        "meaning-gravity",
        "Meaning gravity",
        50 + input.meaning.gravityScore * 5,
        1,
        "Human-need gravity from literal desire to transformed goal.",
      ),
      trace(
        "meaning-confidence",
        "Meaning confidence",
        input.meaning.needConfidence * 100,
        1,
        "Confidence in the mapped positive human need.",
      ),
    );
  }
  return traceEntries;
...
function moduleScore(context: unknown, keys: string[], fallback = 65) {
