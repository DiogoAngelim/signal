...
type NormalizedObservation = Required<
  Pick<RobustnessObservation, "actual" | "confidence" | "participated">
> &
...
export class SignalRobustnessEngine {
  evaluate(input: SignalRobustnessInput): SignalRobustnessResult {
    ...
    const walkForward = evaluateRollingWalkForward(
      ...
      Math.max(
        2,
        Math.round(input.trainWindowSize ?? Math.max(12, minimumSamples)),
      ),
      Math.max(
        1,
        Math.round(
          input.validationWindowSize ??
            Math.max(4, Math.floor(minimumSamples / 3)),
        ),
      ),
      Math.max(
        1,
        Math.round(
          input.stepSize ??
            Math.max(
              1,
              Math.floor(
                (input.validationWindowSize ?? minimumSamples / 3) / 2,
              ),
            ),
        ),
      ),
    ...
    const parameterSensitivity = evaluateParameterSensitivity(
      input.parameterVariants ?? [],
    );
    ...
    const realityGap = estimateRealityGap(
      walkForward.degradationPct,
      adversarial.executionRealismScore,
      statisticalIntegrity.score,
    );
    const observedForwardSamples = Math.max(
      0,
      Math.round(input.observedForwardSamples ?? observations.length),
    );
    const expectedForwardSamples = Math.max(
      1,
      Math.round(input.expectedForwardSamples ?? minimumSamples),
    );
    const forwardSampleScore = clamp(
      (observedForwardSamples / expectedForwardSamples) * 100,
    );
    const historyDepthScore = clamp(
      input.historyDepthScore ?? statisticalIntegrity.minimumSampleScore,
    );
    const regimeCoverageScore = clamp(
      input.regimeCoverageScore ?? regimes.regimeRobustnessScore,
    );
    const regimeDiversityScore = clamp(
      input.regimeDiversityScore ?? clamp(100 - regimes.regimeDependencyScore),
    );
    const sampleDiversityScore = clamp(
      input.sampleDiversityScore ?? statisticalIntegrity.score,
    );
    ...
    const robustnessScore = Math.round(
      weightedMean(
        componentScores,
        [
          1.1, 1, 1, 1.05, 0.9, 0.9, 0.85, 1.25, 0.8, 1, 1.1, 0.7, 0.55, 0.5,
          0.5, 0.45,
        ],
      ),
    );
    const uncertaintyLevel = Math.round(
      clamp(
        100 -
          weightedMean(
            componentScores,
            [
              1, 1, 1.2, 1, 0.8, 0.8, 0.7, 1.4, 0.7, 1, 1.1, 0.7, 0.45, 0.45,
              0.45, 0.4,
            ],
          ),
      ),
    );
    const structuralReliability = Math.round(
      mean([
        parameterSensitivity.stabilityScore,
        leakage.passed ? 100 : 0,
        statisticalIntegrity.score,
    ...
    const generalizationConfidence = Math.round(
      mean([
        robustnessScore,
        calibration.calibrationScore,
    ...
    const safetyGate =
      leakage.passed === false || overfitRisk > 60
        ? "block"
        : overfitRisk > 30 ||
            deploymentReadiness < 62 ||
            participation.participationScore < 35
          ? "reduce"
          : "allow";
...
function normalizeObservations(
  ...
  return observations
    ...
    .sort(
      (a, b) =>
        (a.timestamp ?? a.index ?? a.order) -
        (b.timestamp ?? b.index ?? b.order),
    );
}
...
function evaluateRollingWalkForward(
  ...
  for (
    ...
    const train = observations
      .slice(start, start + trainWindowSize)
      .map((item) => item.actual);
    const validation = observations
      .slice(
        start + trainWindowSize,
        start + trainWindowSize + validationWindowSize,
      )
      .map((item) => item.actual);
    ...
    windows.push({
      ...
      degradationPct:
        trainMean > 0
          ? clamp(
              ((trainMean - validationMean) /
                Math.max(0.0001, Math.abs(trainMean))) *
                100,
              -100,
              100,
            )
  ...
  const consistencyScore = clamp(
    positiveRatio * 100 - stdev(validationMeans) * 6,
  );
  const degradationPct = windows.length
    ? Math.max(0, mean(windows.map((window) => window.degradationPct)))
    : 100;
  const stabilityScore = windows.length
    ? clamp(consistencyScore * 0.7 + (100 - degradationPct) * 0.3)
    : clamp(
        (observations.length /
          Math.max(1, trainWindowSize + validationWindowSize)) *
          55,
      );
...
function evaluateRegimes(observations: NormalizedObservation[]) {
  ...
  const regimes = Array.from(buckets.entries()).map(([regime, values]) => ({
    ...
    score: clamp(
      50 +
        mean(values) * 8 +
        ratio(values, (value) => value > 0) * 35 -
        stdev(values) * 2,
    ),
  }));
  const scores = regimes.map((item) => item.score);
  const sampleCounts = regimes.map((item) => item.samples);
  const dominantShare = observations.length
    ? Math.max(0, ...sampleCounts) / observations.length
    : 1;
  const regimeDependencyScore = clamp(
    dominantShare * 100 + stdev(scores) * 0.6,
  );
  const regimeRobustnessScore = clamp(
    mean(scores) - Math.max(0, dominantShare - 0.55) * 35,
  );
  const adaptabilityScore = clamp(
    regimeRobustnessScore - Math.max(0, stdev(scores) - 18) * 0.8,
  );
...
function evaluateCalibration(
  ...
  const buckets = [0, 20, 40, 60, 80].map((start) => {
    ...
    const values = observations.filter(
      (item) =>
        item.confidence >= start &&
        item.confidence < end + (end === 100 ? 1 : 0),
    );
    const predictedConfidence = values.length
      ? mean(values.map((item) => item.confidence))
      : start + 10;
    const realizedRate = values.length
      ? ratio(
          values.map((item) => item.actual),
          (value) => value > 0,
        ) * 100
      : predictedConfidence;
    return {
  ...
  const weightedError = populated.length
    ? populated.reduce(
        (sum, bucket) =>
          sum +
          Math.abs(bucket.predictedConfidence - bucket.realizedRate) *
            bucket.samples,
        0,
      ) /
      Math.max(
        1,
        populated.reduce((sum, bucket) => sum + bucket.samples, 0),
      )
    : 50;
  const samplePenalty =
    observations.length < minimumSamples
      ? (1 - observations.length / minimumSamples) * 25
      : 0;
...
function evaluateParameterSensitivity(variants: RobustnessParameterVariant[]) {
  ...
  const baseline =
    variants.find((variant) => variant.baselineScore != null)?.baselineScore ??
    mean(variants.map((variant) => variant.score));
  const heatmap = variants.map((variant) => {
    const degradationPct =
      baseline > 0
        ? clamp(
            ((baseline - variant.score) /
              Math.max(0.0001, Math.abs(baseline))) *
              100,
            -100,
            100,
          )
    ...
    return {
      ...
      passed:
        ...
        variant.score >= Math.max(0, variant.benchmarkScore ?? 0),
    };
  });
  const passRate = ratio(heatmap, (item) => item.passed);
  const positiveDegradation = heatmap.map((item) =>
    Math.max(0, item.degradationPct),
  );
  const gradient = clamp(
    mean(positiveDegradation) + stdev(positiveDegradation),
  );
  ...
  const mapped = scenarios.map((scenario) => {
    ...
    return {
      ...
      resilienceScore: clamp(
        100 - Math.max(0, degradationPct) - (scenario.severity ?? 0) * 0.12,
      ),
    };
  });
  return {
    robustnessScore: mean(mapped.map((item) => item.resilienceScore)),
    executionRealismScore: clamp(
      mean(mapped.map((item) => item.resilienceScore)) -
        Math.max(0, ...mapped.map((item) => item.degradationPct)) * 0.15,
    ),
    scenarios: mapped,
  };
}
...
function evaluateParticipation(observations: NormalizedObservation[]) {
  ...
  const opportunityDensity = observations.length
    ? participated.length / observations.length
    : 0;
  const executableConviction = participated.length
    ? clamp(
        ratio(
          participated.map((item) => item.actual),
          (value) => value > 0,
        ) *
          75 +
          mean(participated.map((item) => item.confidence)) * 0.25,
      )
    : 0;
  const participationScore = clamp(
    opportunityDensity * 70 + executableConviction * opportunityDensity * 0.3,
  );

  return {
    participationScore,
    opportunityDensity: opportunityDensity * 100,
    executableConviction,
  };
}
...
function evaluateEnsemble(votes: RobustnessEnsembleVote[]) {
  ...
  const totalWeight =
    votes.reduce((sum, vote) => sum + Math.max(0, vote.weight ?? 1), 0) || 1;
  const weightedDirection =
    votes.reduce(
      (sum, vote) =>
        sum +
        Math.sign(vote.direction) *
          clamp(vote.confidence) *
          Math.max(0, vote.weight ?? 1),
      0,
    ) / totalWeight;
  const largestWeight =
    Math.max(...votes.map((vote) => Math.max(0, vote.weight ?? 1))) /
    totalWeight;
  const dominancePenalty = clamp(Math.max(0, largestWeight - 0.45) * 100);
  const disagreementScore = clamp(
    100 -
      stdev(
        votes.map((vote) => Math.sign(vote.direction) * clamp(vote.confidence)),
      ) *
        0.9,
  );
  const consensusScore = clamp(
    Math.abs(weightedDirection) -
...
function evaluateLeakage(checks: LeakageCheck[]) {
  const violations = checks.flatMap((check) => {
    const issues: string[] = [];
    if (check.validationStartIndex <= check.trainEndIndex)
      issues.push(`${check.id}: validation overlaps training`);
    if (check.normalizedWithFuture === true)
      issues.push(`${check.id}: normalization used future data`);
    if (
...
function evaluateFeatureTrust(observations: NormalizedObservation[]) {
  const keys = Array.from(
    new Set(observations.flatMap((item) => Object.keys(item.features ?? {}))),
  );
  if (!keys.length) {
  ...
  const features = keys.map((key) => {
    const pairs = observations
      ...
      .filter(
        ([feature, actual]) =>
          Number.isFinite(feature) && Number.isFinite(actual),
      );
    const trust =
      pairs.length >= 3
        ? clamp(
            50 +
              correlation(
                pairs.map(([feature]) => feature as number),
                pairs.map(([, actual]) => actual),
              ) *
                50,
          )
        : 45;
  ...
  const total =
    features.reduce((sum, item) => sum + Math.max(0, item.weight), 0) || 1;
  const probabilities = features.map(
    (item) => Math.max(0, item.weight) / total,
  );
  const entropy = clamp(
    (-probabilities.reduce(
      (sum, value) => sum + (value > 0 ? value * Math.log2(value) : 0),
      0,
    ) /
  ...
  const trustScore = clamp(
    mean(features.map((item) => item.trust)) - Math.max(0, 35 - entropy) * 0.4,
  );
...
function estimateRealityGap(
  degradationPct: number,
  executionRealismScore: number,
  statisticalIntegrityScore: number,
) {
  const expectedDegradationPct = clamp(
    Math.max(0, degradationPct) * 0.45 +
      (100 - executionRealismScore) * 0.25 +
      (100 - statisticalIntegrityScore) * 0.18,
  );
  return {
...
function buildReasons(input: {
  ...
  if (input.overfitRisk > 30)
    reasons.push("Overfit risk is above the production threshold.");
  if (input.walkForward < 60)
    reasons.push("Rolling walk-forward validation is unstable.");
  if (input.regimes < 60)
    reasons.push("Performance is too dependent on a narrow regime set.");
  if (input.calibration < 60)
    reasons.push(
      "Predicted confidence is poorly calibrated to realized outcomes.",
    );
  if (input.parameters < 60)
    reasons.push("Nearby parameter variants are fragile.");
  if (input.adversarial < 60)
    reasons.push("Adversarial distortions create too much degradation.");
  if (input.participation < 35)
    reasons.push("Executable participation is too sparse.");
  if (!input.leakage.passed) reasons.push(...input.leakage.violations);
  if (input.statisticalIntegrity < 60)
    reasons.push("Statistical integrity is below the production floor.");
  if (input.historyDepthScore < 55)
    reasons.push(
      "Historical depth is too shallow for broad overfit confidence.",
    );
  if (input.regimeCoverageScore < 55)
    reasons.push("Regime coverage is too narrow for broad overfit confidence.");
  if (input.regimeDiversityScore < 55)
    reasons.push(
      "Regime diversity is too narrow for broad overfit confidence.",
    );
  if (input.sampleDiversityScore < 55)
    reasons.push(
      "Sample diversity is too concentrated for broad overfit confidence.",
    );
  if (input.safetyGate === "allow" && reasons.length === 0)
    reasons.push("Robustness diagnostics are within production tolerance.");
...
function inferRegime(
  ...
  const volatility = finite(
    features?.volatility ?? features?.risk ?? Math.abs(actual),
  );
...
function bootstrapPositiveMeanStability(values: number[], seed: number) {
  ...
  for (let round = 0; round < rounds; round += 1) {
    const sample = values.map(
      () => values[Math.floor(generator() * values.length)],
    );
...
function monteCarloOrderStability(values: number[], seed: number) {
  ...
  for (let round = 0; round < rounds; round += 1) {
    ...
    if (
      ...
      trough < Math.max(12, Math.abs(curve[curve.length - 1]) * 0.65)
    )
      stable += 1;
  }
...
function weightedMean(values: number[], weights: number[]) {
  const totalWeight =
    values.reduce((sum, _value, index) => sum + (weights[index] ?? 1), 0) || 1;

  return (
    values.reduce(
      (sum, value, index) => sum + clamp(value) * (weights[index] ?? 1),
      0,
    ) / totalWeight
  );
}
