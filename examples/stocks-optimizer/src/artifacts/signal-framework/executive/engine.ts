...
export type ExecutiveParticipationMode =
  | "none"
  | "watch"
  | "limited"
  | "normal"
...
export function evaluateExecutiveDecision(
  ...
  const states = evaluateDecisionStates({
    ...
    restrictions,
    trustGovernor:
      input.readiness?.trustGovernor ??
      input.agency?.trustGovernor ??
      input.resolve?.trustGovernor,
  ...
  const decision = finalDecisionFor(
    input,
    baseDecision,
    states,
    confidence,
  ...
  const invalidationConditions = conditionsFor(
    ...
    [
      ...
      decision === "buy"
        ? "Invalidate if permission, execution quality, or trust falls below the action threshold."
        : "",
    ],
  );
  const primaryReason = primaryReasonFor(
    input,
    decision,
    strongestEvidence,
    primaryLimiter,
  );
  const nextReviewCondition =
    input.nextReviewCondition ??
    nextReviewConditionFor(input, states, decision);
...
function normalizeRestrictions(input: ExecutiveInput): DecisionRestriction[] {
  return [
    ...
    ...(input.discoveryAccountability?.blockers ?? []).map(
      (blocker, index) => ({
...
function componentScoresFor(input: ExecutiveInput, trust: TrustState) {
  return {
    ...
    discoveryAccountability: score(
      input.discoveryAccountability?.accountabilityScore,
      50,
    ),
    discoveryIntelligence: score(input.discoveryIntelligence?.score, 50),
    recognition: score(
      input.recognition?.recognitionScore ?? input.recognition?.confidence,
      50,
    ),
    belief: score(input.belief?.confidence, 50),
    judgement: score(
      input.judgement?.adjustedConfidence ?? input.judgement?.reliability,
      50,
    ),
    agency: score(input.agency?.trustPct ?? input.agency?.trust, 50),
    wisdom: score(
      input.wisdom?.wisdomScore ?? input.wisdom?.decisionQuality,
      50,
    ),
...
function baseDecisionFor(input: ExecutiveInput): ExecutiveAction {
  ...
  if (resolveDecision === "commit")
    return input.opportunity != null && input.opportunity < 40 ? "hold" : "buy";
  if (resolveDecision === "reject" || resolveDecision === "invalidate")
    return "avoid";
  ...
  if (
    agencyRecommendation.includes("reject") ||
    agencyRecommendation.includes("block")
  )
    return "avoid";
  if (
    agencyRecommendation.includes("act") ||
    agencyRecommendation.includes("commit")
  )
    return "buy";
...
function finalDecisionFor(
  ...
  if (!states.permission.allowed) {
    ...
    return states.permission.level === "review_required"
      ? "review"
      : baseDecision === "sell"
        ? "sell"
        : "avoid";
  ...
  if (
    ...
    (baseDecision === "buy" || baseDecision === "sell")
  ) {
    trace.push(
      "Execution quality is poor, so action is held for cleaner execution.",
    );
    return "hold";
  }
...
function participationModeFor(
  ...
  if (
    states.capacity.mode === "micro" ||
    states.capacity.mode === "reduced" ||
    states.permission.level === "limited"
  )
    return "limited";
  if (
    states.capacity.mode === "expanded" &&
    states.trust.status === "highly_trusted" &&
    states.urgency.mode === "act_now" &&
    confidence >= 82
  )
    return "aggressive";
...
function strongestEvidenceFor(
  ...
  const evidence = [
    ...
    ...toStringArray(
      input.discovery?.explanation?.supportingEvidence?.map(
        (item: any) => item.reason ?? item.label,
      ),
    ),
    ...toStringArray(
      input.belief?.supportingEvidence?.map(
        (item: any) => item.reason ?? item.name,
      ),
    ),
  ...
  const scoredEvidence = Object.entries(scores)
    ...
    .map(
      ([key, value]) =>
        `${labelize(key)} contributes ${Math.round(value)}/100.`,
    );
...
function primaryLimiterFor(
  ...
  return firstString(
    ...
    states.capacity.mode !== "normal" && states.capacity.mode !== "expanded"
      ? states.capacity.reasons[0]
      : "",
    input.wisdom && input.wisdom.wisdomScore < 45
      ? input.wisdom.explanation
      : "",
    input.discoveryIntelligence && input.discoveryIntelligence.score < 45
      ? input.discoveryIntelligence.recommendations[0]?.message
      : "",
...
function conditionsFor(
  values: ExecutiveInput["unlockConditions"],
  source: string,
  fallbacks: unknown[],
): DecisionCondition[] {
  const explicit = (Array.isArray(values) ? values : []).map(
    (condition, index) => normalizeCondition(condition, source, index),
  );
  const fallback = fallbacks
    .flatMap((item) => toStringArray(item))
    .map((description, index) =>
      normalizeCondition(description, source, explicit.length + index),
    );
...
function normalizeCondition(
  value: DecisionCondition | string | undefined,
  source: string,
  index: number,
): DecisionCondition {
...
function nextReviewConditionFor(
  ...
  if (decision === "review")
    return "Review immediately because permission requires human or policy review.";
  if (states.urgency.mode === "act_now" || states.urgency.mode === "act_soon")
    return "Review after the next execution-quality or risk update.";
  if (input.wisdom && input.wisdom.learningConfidence < 45)
    return "Review when Wisdom records more comparable outcomes or counterfactuals.";
  if (input.discoveryIntelligence && input.discoveryIntelligence.score < 45)
    return "Review when Discovery Intelligence shows the learning loop is creating value.";
  if (input.discoveryAccountability?.status === "immature")
    return "Review when discovery accountability matures or confirms recurrence.";
...
function normalizeAction(value: unknown): ExecutiveAction | null {
  ...
  if (
    ...
    return action as ExecutiveAction;
  return null;
}
