...
describe("calibration", () => {
  ...
  it("detects overconfidence, underconfidence, and aligned confidence", () => {
    ...
    expect(calibrateConfidence([
      ...
      trace(0.2, false),
      trace(0.1, false),
    ], { alignmentTolerance: 0.15 })).toMatchObject({
  ...
  it("honors calibration configuration and validates invalid values", () => {
    expect(calibrateConfidence([
      trace(0.9, false),
      trace(0.9, false),
    ], { minimumSamples: 2, adjustmentRate: 1 })).toMatchObject({
      calibratedConfidence: 0,
      calibrationError: 0.9,
      reliability: "overconfident",
    });

    expect(() => calibrateConfidence([trace(0.5, true)], { minimumSamples: 0 })).toThrow(
      "minimumSamples must be a positive integer.",
    );
    expect(() => calibrateConfidence([trace(0.5, true)], { alignmentTolerance: 2 })).toThrow(
