...
function buildSummary(
  ...
  if (status === "Watch") {
    return `${symbol} moved ${direction} ${magnitude}% today — elevated volatility, exercise caution.`;
  }
...
function buildImpact(
  ...
  if (status === "Watch") {
    return "Elevated volatility detected. Consider tighter position sizing and stop-loss levels.";
  }
  if (status === "Rising") {
    return "Bullish momentum building. Monitor for a continuation or retracement entry.";
  }
  if (status === "Dip") {
    return "Bearish pressure present. Watch key support levels for a potential reversal.";
  }
  if (Math.abs(changePercent) < 0.5) {
    return "Pair consolidating in a tight range. No immediate directional bias.";
  }
  return "Movement within expected daily range. No immediate action required.";
}
