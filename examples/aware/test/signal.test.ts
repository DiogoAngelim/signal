...
import { createFixtureAwareAdapters, createRegionService } from "../src/adapters.js";
...
describe("Aware Signal integration", () => {
  it("exposes the required query, mutation, and event contracts", () => {
    ...
    expect(contracts.filter((contract) => contract.kind === "query").map((contract) => contract.name)).toEqual(AWARE_OPERATION_NAMES.queries);
    expect(contracts.filter((contract) => contract.kind === "mutation").map((contract) => contract.name)).toEqual(AWARE_OPERATION_NAMES.mutations);
    expect(contracts.filter((contract) => contract.kind === "event").map((contract) => contract.name)).toEqual(AWARE_OPERATION_NAMES.events);
    expect(contracts.find((contract) => contract.name === "aware.feedback.submit.v1")?.idempotency).toBe("required");
  });
  ...
  it("keeps degraded-mode responses cautious", async () => {
    ...
    expect(briefing.summary).toBe("Evidence is limited, so this guidance is cautious.");
    expect(briefing.items.some((item) => item.fallbackBehavior.includes("cautious"))).toBe(true);
    expect(app.events.list().map((event) => event.name)).toContain("aware.source.degraded.v1");
  });
  ...
  it("records feedback idempotently", async () => {
    ...
    expect(app.events.list().filter((event) => event.name === "aware.feedback.received.v1")).toHaveLength(1);
  });
});
