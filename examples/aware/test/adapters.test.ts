...
describe("Aware adapters", () => {
  it("searches supported regions with beginner-friendly labels", async () => {
    const regions = await createRegionService({ addressLookup: false }).search(
      "miami",
    );
  ...
  it("uses the address API when a query is not in the fixed region catalog", async () => {
    ...
    expect(
      createRegionService({ addressLookup: false }).get(regions[0]?.id),
    ).toMatchObject({
      name: "Washington",
      latitude: 38.8977,
    });
  ...
  it("normalizes weather, air, exposure, alert, and mosquito facts without recommendations", async () => {
    ...
    expect(
      result.observations.map((observation) => observation.category),
    ).toEqual(
    ...
    expect(
      result.sources.every(
        (source) =>
          source.provider === "fixture" || source.provider === "derived",
      ),
    ).toBe(true);
    expect(
      result.observations.every(
        (observation) => !("primaryAction" in observation),
      ),
    ).toBe(true);
    expect(
      result.observations.find(
        (observation) => observation.signal === "weather.uv",
      )?.severity,
    ).toBe(2);
  });
  ...
  it("marks unavailable sources as degraded evidence instead of pretending the day is complete", async () => {
    ...
    expect(
      result.observations.some(
        (observation) =>
          observation.category === "source_status" && observation.missing,
      ),
    ).toBe(true);
  });
});
