...
import { type AwareSignalApp, createAwareSignalApp } from "../signal.js";
...
export function createAwareApiService(
  options: AwareApiServiceOptions = {},
): AwareApiService {
  ...
  return {
    app: createAwareSignalApp({
      ...
      adapters:
        options.adapters ??
        (options.fixtureId
          ? createFixtureAwareAdapters(options.fixtureId)
          : createDefaultAwareAdapters()),
      now: options.now,
    }),
  };
...
export async function getRegionBriefing(
  service: AwareApiService,
  regionId: string,
  fixtureId?: FixtureScenarioId,
) {
...
export async function getBriefingDetails(
  service: AwareApiService,
  briefingId: string,
) {
...
export async function listBriefingSources(
  service: AwareApiService,
  briefingId: string,
) {
