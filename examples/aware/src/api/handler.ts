...
export async function handleAwareApiRequest(
  ...
  try {
    if (request.method === "GET" && path === "/api/regions/search") {
      ...
      return json({ ok: true, data: { regions: await searchRegions(service, query) } });
    }

    const regionBriefingMatch = path.match(/^\/api\/regions\/([^/]+)\/briefing$/);
    ...
    const briefingSourcesMatch = path.match(/^\/api\/briefings\/([^/]+)\/sources$/);
    if (request.method === "GET" && briefingSourcesMatch?.[1]) {
      ...
      if (!result.found) return json(error("NOT_FOUND", "Briefing was not found."), 404);
    ...
    const message = caught instanceof Error ? caught.message : "Aware API request failed.";
