...
export function createOfficialAlertsAdapter(
...
async function collectLiveAlerts(
  region: Region,
  context: AdapterContext,
  ...
  const updatedAt = properties?.effective
    ? new Date(properties.effective).toISOString()
  ...
  return {
    ...
    observations: [
      createObservation({
        ...
        validUntil: properties?.expires
          ? new Date(properties.expires).toISOString()
          : undefined,
        severity,
        source,
        plainLanguage:
          severity > 0
            ? (properties?.headline ??
              "An official weather alert is active for this area.")
  ...
  return {
    ...
    observations: [
      createObservation({
        ...
        validUntil: new Date(
          new Date(scenario.updatedAt).getTime() + 12 * 60 * 60 * 1000,
        ).toISOString(),
        severity: scenario.officialAlerts.severity,
        source,
        plainLanguage:
          scenario.officialAlerts.severity > 0
