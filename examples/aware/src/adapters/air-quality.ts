...
async function collectLiveAirQuality(
  region: Region,
  context: AdapterContext,
  ...
  const usAqi = numberOrZero(
    payload.current?.us_aqi ?? payload.hourly?.us_aqi?.[0],
  );
  const pm25 = numberOrZero(
    payload.current?.pm2_5 ?? payload.hourly?.pm2_5?.[0],
  );
...
function collectFixtureAirQuality(
  region: Region,
  context: AdapterContext,
): AdapterRunResult {
  const scenario = scenarioFor(region, context);
...
function fromAirValues(
  ...
  const severity =
    input.severity ??
    severityFromThresholds(input.usAqi, {
      notice: 75,
      warning: 125,
      urgency: 175,
      emergency: 250,
  ...
  return {
    ...
    observations: [
      createObservation({
        ...
        validUntil: new Date(
          new Date(input.observedAt).getTime() + 6 * 60 * 60 * 1000,
        ).toISOString(),
        severity,
        source: input.source,
        plainLanguage:
