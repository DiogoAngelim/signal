...
async function collectLiveWeather(
  region: Region,
  context: AdapterContext,
  ...
  const updatedAt = payload.current?.time
    ...
    : context.now().toISOString();
  const maxTemperatureC = numberOrZero(
    payload.daily?.temperature_2m_max?.[0] ?? payload.current?.temperature_2m,
  );
  const precipitationMm = numberOrZero(
  ...
  const heatSeverity = severityFromThresholds(maxTemperatureC, {
    notice: 30,
    warning: 35,
    urgency: 40,
    emergency: 45,
  });
  const rainSeverity = severityFromThresholds(precipitationMm, {
    notice: 10,
    warning: 35,
    urgency: 70,
    emergency: 125,
  });
  const uvSeverity = severityFromThresholds(uvIndex, {
    notice: 6,
    warning: 8,
    urgency: 11,
  });
...
function collectFixtureWeather(
  region: Region,
  context: AdapterContext,
): AdapterRunResult {
