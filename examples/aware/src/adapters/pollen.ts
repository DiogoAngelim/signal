...
export function createPollenAdapter(context: AdapterContext): SafetyDataAdapter {
  return {
...
async function collectLivePollen(region: Region, context: AdapterContext): Promise<AdapterRunResult | undefined> {
  ...
  const updatedAt = payload.hourly?.time?.[0] ? new Date(payload.hourly.time[0]).toISOString() : context.now().toISOString();
...
function collectFixturePollen(region: Region, context: AdapterContext): AdapterRunResult {
...
function fromPollenValues(region: Region, context: AdapterContext, input: {
  ...
  const severity = input.severity ?? severityFromThresholds(input.index, {
    notice: 4,
    warning: 7,
    urgency: 10
  });
  return {
    ...
    observations: [
      createObservation({
        ...
        validUntil: new Date(new Date(input.observedAt).getTime() + 24 * 60 * 60 * 1000).toISOString(),
        severity,
        source: input.source,
