...
export type NubankConnectApiResult =
  | {
      ...
      rawTransactions: Awaited<ReturnType<NubankConnector["fetchTransactions"]>>;
      message?: string;
    }
  | {
      ok: false;
...
export async function handleLiquidityManagerApiRequest(
  ...
  if (url.pathname === "/api/local-nubank-statement" && request.method === "GET") {
  ...
  if (url.pathname === "/api/nubank/last-attempt" && request.method === "GET") {
    return jsonResponse({
      ...
      message: lastNubankAttempt ? "Last Nubank attempt is available." : "No Nubank sync attempt has run yet."
    });
  }

  return jsonResponse({ ok: false, message: "Route not found." }, { status: 404 });
}
...
async function handleNubankConnect(
  ...
  if (!result.ok) {
    const message = result.message ?? "Could not connect to Nubank automatically. Upload a Nubank statement instead.";
    rememberNubankAttempt({
      status: "failed",
      statusCode: 502,
      message
  ...
  rememberNubankAttempt({
    status: "success",
    statusCode: 200,
    message: result.message ?? "Nubank transactions imported through the QR-authorized connector.",
...
function rememberNubankAttempt(attempt: Omit<SafeNubankAttempt, "updatedAt">): void {
  lastNubankAttempt = {
    ...attempt,
