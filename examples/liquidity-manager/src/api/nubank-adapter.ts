...
type NubankApi = {
  getLoginToken?: (input: { login: string; password: string }) => Promise<unknown>;
...
export function createUnofficialNubankSessionAdapter({
  ...
  return {
    async createSession({ cpf, password }) {
      ...
      if (!password) {
        throw new NubankConnectionError("The legacy Nubank client requires CPF and password.");
      }
      if (typeof api.getLoginToken !== "function") {
        throw new NubankConnectionError("The unofficial Nubank client does not expose a login method.");
      }
      if (typeof api.getWholeFeed !== "function") {
        throw new NubankConnectionError("The unofficial Nubank client does not expose a transaction feed method.");
      }
...
export function resolveNubankFactory(moduleValue: unknown): () => unknown {
  ...
  throw new NubankConnectionError("The unofficial Nubank client could not be loaded.");
}

async function requestLoginToken(api: NubankApi, cpf: string, password: string): Promise<unknown> {
  try {
    return await api.getLoginToken?.({ login: cpf.replace(/\D/g, ""), password });
  } catch {
    throw new NubankConnectionError("Nubank login failed. Check the CPF, password, and any confirmation required by Nubank.");
  }
}
...
async function requestTransactionFeed(api: NubankApi): Promise<unknown> {
  try {
    ...
    throw new NubankConnectionError("Nubank login completed, but the unofficial client could not fetch the transaction feed link.");
  }
}

function assertSignedInSession(sessionData: unknown): void {
  if (!isRecord(sessionData)) {
    throw new NubankConnectionError("Nubank login failed. The unofficial client did not receive a valid session.");
  }
  ...
  if (typeof eventsHref !== "string" || !eventsHref) {
    throw new NubankConnectionError("Nubank login completed, but Nubank did not return a transaction feed link for this unofficial client.");
  }
}
...
function normalizeNubankFeedItem(item: unknown, index: number): RawTransaction | undefined {
  ...
  const id = readString(item, ["id", "uuid", "href"]) ?? `nubank-${index + 1}-${date.toISOString().slice(0, 10)}`;
...
async function readAvailableBalance(api: NubankApi): Promise<number | undefined> {
  for (const method of ["getCustomerAccount", "getAccountBalance", "getBalance", "getSavingsAccountBalance"] as const) {
    const fn = api[method];
    if (typeof fn !== "function") continue;
    try {
      const result = await fn.call(api);
      const amount = extractFirstNumber(result, ["available", "availableAmount", "balance", "amount", "value"]);
...
function readAmount(item: Record<string, unknown>): number | undefined {
  const explicitAmount = extractFirstNumber(item, ["amount", "value", "total", "details.amount", "details.value"]);
...
function readDate(item: Record<string, unknown>): Date | undefined {
  const value = readString(item, ["date", "time", "postDate", "createdAt", "details.date"]);
...
function readDescription(item: Record<string, unknown>): string {
  return (
    readString(item, ["description", "title", "label", "merchant", "details.description", "details.title", "details.subtitle"]) ??
    "Nubank transaction"
  );
}

function inferSignedAmount(amount: number, item: Record<string, unknown>): number {
  if (amount < 0) return amount;
  const category = `${readString(item, ["category", "kind", "type", "title", "description"]) ?? ""}`.toLowerCase();
  const inflowWords = ["payment", "pagamento recebido", "deposit", "deposito", "depósito", "cashback", "refund", "estorno"];
  const outflowWords = ["transaction", "purchase", "compra", "pix", "transfer", "debit", "débito"];
  if (inflowWords.some((word) => category.includes(word))) return Math.abs(amount);
  if (outflowWords.some((word) => category.includes(word))) return -Math.abs(amount);
  return amount;
}
