...
export function calculateCashflowProfile({
  ...
  const latestBalance = [...balances].sort(
  ...
  const currentBalance = roundCurrency(
    latestBalance?.availableAmount ?? Math.max(0, derivedBalance),
  );
  const firstDate = sorted[0]?.date ?? now;
  const lastDate = sorted[sorted.length - 1]?.date ?? now;
  const dataCoverageDays = Math.max(
    0,
    Math.ceil(
  ...
  const incomePredicate = (transaction: NormalizedTransaction) =>
    transaction.type === "income" || transaction.type === "refund";
  const expensePredicate = (transaction: NormalizedTransaction) =>
  ...
  const fixedMonthlyExpenses = roundCurrency(
    detectFixedMonthlyExpenses(sorted, dataCoverageDays),
  );
  const discretionaryMonthlyExpenses = roundCurrency(
    Math.max(0, averageMonthlyExpenses - fixedMonthlyExpenses),
  );
...
export function calculateScenarioSnapshot({
  ...
  const weeklyNetBurn =
    Math.max(0, averageMonthlyExpenses - averageMonthlyIncome) /
    WEEKS_PER_MONTH;
  const weeklyBurn = Math.max(1, weeklyExpenseBurden, weeklyNetBurn);
  const runwayWeeks = roundOne(
    clamp(currentBalance / weeklyBurn, 0, MAX_RUNWAY_WEEKS),
  );
  const volatility = incomeVolatility * 0.55 + expenseVolatility * 0.45;

  return {
    runwayWeeks,
    shortfallRisk30d: roundRisk(
      shortfallRisk({
        currentBalance,
        averageMonthlyIncome,
        averageMonthlyExpenses,
...
function sumTransactions(
  ...
  return transactions
    .filter(
      (transaction) =>
        transaction.direction === direction && predicate(transaction),
    )
...
function detectFixedMonthlyExpenses(
  transactions: NormalizedTransaction[],
  dataCoverageDays: number,
): number {
  const groups = new Map<string, NormalizedTransaction[]>();
  for (const transaction of transactions) {
    if (transaction.direction !== "outflow" || transaction.type !== "expense")
      continue;
...
function shortfallRisk({
  ...
  const projectedBalance =
    currentBalance + (averageMonthlyIncome - averageMonthlyExpenses) * months;
