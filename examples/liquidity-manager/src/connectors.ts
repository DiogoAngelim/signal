...
import { type ManualUploadInput, parseManualCsv } from "./manual-upload.js";
import type {
  BalanceSnapshot,
  BankConnection,
  ConnectionResult,
...
export class SampleDataConnector implements FinancialDataConnector {
  ...
  async connect(input: unknown): Promise<ConnectionResult> {
    ...
    this.dataset = createSampleFinancialDataset({
      userId,
  ...
  async fetchBalances(connectionId: string): Promise<BalanceSnapshot[]> {
    return this.dataset?.connectionId === connectionId
      ? this.dataset.balances
      : [];
  }
...
export class ManualUploadConnector implements FinancialDataConnector {
  ...
  private records = new Map<
    string,
    { balances: BalanceSnapshot[]; transactions: RawTransaction[] }
  >();
  private readonly now: Clock;
  ...
  async connect(input: unknown): Promise<ConnectionResult> {
    ...
    const currentBalance =
      typed.currentBalance ??
      parsed.currentBalance ??
      deriveBalance(parsed.transactions);
    ...
    return {
      ok: true,
      connection,
      message: `${parsed.transactions.length} transactions imported.`,
    };
  }
...
export class NubankConnector implements FinancialDataConnector {
  ...
  async connect(input: unknown): Promise<ConnectionResult> {
    const typed = input as
      | { userId?: string; cpf?: string; password?: string; authCode?: string }
      | undefined;
    const userId = readUserId(input);
    const connection = createConnection({
      userId,
    ...
    if (!typed?.cpf) {
      return {
        ...
        connection: {
          ...connection,
          status: "failed",
          maskedCpf: typed?.cpf ? maskCpf(typed.cpf) : undefined,
        },
    ...
    if (!this.adapter) {
      return {
        ...
        message:
          "Nubank connector adapter is not configured. Upload a Nubank statement instead.",
      };
    }
    ...
    try {
      const session = await this.adapter.createSession({
        ...
        authCode: typed.authCode,
      });
      const connected: BankConnection = {
        ...connection,
        maskedCpf: maskCpf(typed.cpf),
        encryptedSessionData: await encryptSessionData(
          session.sessionData,
