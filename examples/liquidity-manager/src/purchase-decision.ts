...
export function createPurchaseDecision({
  ...
  const installmentPayment =
    input.paymentMethod === "installments" ? input.amount / installments : 0;
  const oneMonthCreditImpact =
    input.paymentMethod === "credit" ? input.amount : 0;
  const immediateBalanceDelta =
    input.paymentMethod === "cash" ? -input.amount : 0;
  ...
  const scoreParts = {
    ...
    volatility:
      clamp(
        1 -
          profile.incomeVolatility -
          input.amount / Math.max(1, profile.averageMonthlyIncome * 3),
        0,
        1,
      ) * 100,
    necessity: necessityScore(input.necessity),
    recentSpending: recentSpendingScore({
      transactions,
      amount: input.amount,
      now,
      monthlyExpenses: profile.averageMonthlyExpenses,
    }),
...
export function calculateDecisionConfidence(profile: CashflowProfile): number {
  ...
  if (profile.dataCoverageDays >= 180 && profile.transactionCount >= 100)
    return 95;
  if (profile.dataCoverageDays >= 90 && profile.transactionCount >= 60)
    return 80;
  if (profile.dataCoverageDays >= 60 && profile.transactionCount >= 30)
    return 60;
  return 40;
}
  ...
  const recentOutflow = transactions
    .filter(
      (transaction) =>
        ...
        transaction.date.getTime() >= cutoff,
    )
...
function recoveryScore({
  amount,
  profile,
}: { amount: number; profile: CashflowProfile }): number {
  const monthlySurplus =
    profile.averageMonthlyIncome - profile.averageMonthlyExpenses;
...
function applyHardCaps({
  ...
  if (input.paymentMethod === "cash" && input.amount > profile.currentBalance)
    capped = Math.min(capped, 49);
  if (
    input.paymentMethod === "installments" &&
    profile.averageMonthlyExpenses + installmentPayment >
      profile.averageMonthlyIncome
  ) {
...
function verdictForScore(
  ...
  if (score >= 50) {
    if (input.paymentMethod === "cash" && after.runwayWeeks < SAFE_RUNWAY_WEEKS)
...
function explainDecision({
  ...
  const runwayDelta =
    Math.round((before.runwayWeeks - after.runwayWeeks) * 10) / 10;

  if (verdict === "approved")
    return `${confidencePrefix}Approved: your runway remains above target.`;
  if (verdict === "acceptable")
    return `${confidencePrefix}Acceptable: this weakens your buffer by ${runwayDelta} weeks, but remains manageable.`;
  if (verdict === "delay")
    return `${confidencePrefix}Delay: this purchase would be safer after your next income cycle.`;
  if (verdict === "reduce_amount")
    return `${confidencePrefix}Reduce amount: ${formatBrl(input.amount)} would push runway below the target buffer.`;
  if (verdict === "risky")
    return `${confidencePrefix}Risky: this materially increases shortfall risk.`;
...
function saferAlternative({
  ...
  const safeAmount = Math.max(
    0,
    Math.floor((profile.currentBalance - safeBalanceFloor) / 10) * 10,
  );

  if (
    input.paymentMethod === "cash" &&
    safeAmount > 0 &&
    safeAmount < input.amount
  ) {
    return `Reduce the purchase to ${formatBrl(safeAmount)}.`;
  }

  if (after.runwayWeeks < SAFE_RUNWAY_WEEKS)
    return "Delay until runway is back above 8 weeks.";
